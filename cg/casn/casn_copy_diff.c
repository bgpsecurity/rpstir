    ...
    struct casn *fr_casnp)
{
    ...
    if (_clear_error(to_casnp) < 0 || _clear_error(fr_casnp) < 0)
        return -1;
    ...
    struct casn *casnp2)
{
    ...
    if ((casnp1->type != casnp2->type && casnp2->type != ASN_NOTASN1 &&
        ...
        !(casnp1->flags & ASN_FILLED_FLAG)
        || !(casnp2->flags & ASN_FILLED_FLAG))
        return -2;
    if (casnp1->type == ASN_INTEGER || casnp1->type == ASN_ENUMERATED ||
        casnp1->type == ASN_BOOLEAN)
    {
        if (!casnp1->startp || !casnp2->startp)
            return -2;
        if (casnp1->type == ASN_INTEGER)
        {
            ...
            neg = 0;
        if (casnp1->lth > casnp2->lth)
            diff = 1;
        else if (casnp1->lth < casnp2->lth)
            diff = -1;
        else
            diff = memcmp(casnp1->startp, casnp2->startp, casnp1->lth);
        if (neg)
            diff = -diff;
    }
        ...
        for (casnp1++, casnp2++; casnp1 && casnp2;
             casnp1 = _skip_casn(casnp1, 1), casnp2 = _skip_casn(casnp2, 1))
        {
            if ((diff = _diff_casn(casnp1, casnp2, 1)))
                break;
        }
    return diff;
}
...
**/
            ...
            if (!err && (to_casnp->flags & ASN_DEFINED_FLAG))
            {
                ...
                if (to_casnp->type == ASN_ANY)
                    to_casnp->tag = fr_casnp->tag;
                     ...
                     (to_casnp =
                      _find_tag(&(tcasnp = to_casnp)[1], fr_casnp->tag)))
                err = ASN_MATCH_ERR;
        }
    ...
    if (err)
        return _casn_obj_err(tcasnp, err);
    // step 2
    tcasnp = fr_casnp;
    ...
    {
        if ((fr_casnp->flags & ASN_OPTIONAL_FLAG))
            return 0;
    ...
    if ((fr_casnp->flags & ASN_OF_FLAG))
    {
        ...
        else
        {
            ...
            {
                if (!(tcasnp = inject_casn(to_casnp, num)))
                    return -did;
