...
static int shift_buf(
    uchar *,
    int,
    int);
static void swap_box(
    uchar *);
    ...
    double *val)
{
    ...
    **/
    ...
    if (casnp->level > 0 && (_go_up(casnp)->flags & ASN_OF_FLAG) &&
        (casnp->flags & ASN_LAST_FLAG))
        i = ASN_OF_BOUNDS_ERR;
    else if (!casnp->startp)
        i = ASN_MANDATORY_ERR;
    if (i)
    ...
    if (!(tc & 0xC0))
    {
        ...
        locbuf[casnp->lth] = 0;
        *val = 0;
        for (c = &locbuf[1]; *c && *c != ','; c++);     // convert comma as
                                                        // decimal
        if (*c)
            *c = '.';           // mark to period
        sscanf((char *)&locbuf[1],
               (tc == ((ISO6093NR3 - 8) / 2)) ? "%le" : "%lf", &box.dbl_val);
    }
    else
    {
        sign = (tc & 0x40);
        ...
        if (locbuf[i] < mask)   // can shift all the way
        {
            ...
            exponent -= 8;
            // how much left shift will bring mantissa's MSbit to proper
            // point?
        ...
        *locbuf &= DBL_MMSBYTE_MASK;
        ...
        if (*bigend.cval)
            swap_box(box.cval);
        if (sign)
            box.dbl_val = -box.dbl_val;
    }
    *val = box.dbl_val;
    return 1;
}

int write_casn_double(
    struct casn *casnp,
    double val,
    int type)
{
    ...
    **/
    ...
    else                        // general case 2
    {
        ...
        *mantissap &= DBL_MMSBYTE_MASK;
        ...
        if (!*mantissap)
            memcpy(mantissap, &mantissap[1], mantissa_lth--);
        if (mantissa_lth < 0)
            return _casn_obj_err(casnp, ASN_GEN_ERR);
        // step 5
