...
void AlgorithmTableInAlgorithmIdentifier(struct casn *mine, ushort level)
    ...
    mine->level = level;
    mine->ptr = (struct casn *)calloc(54, sizeof(struct casn));
    tcasnp = mine->ptr;
    ...
    tcasnp++;
    tcasnp->type = ASN_OBJ_ID;
    tcasnp->lth = _write_objid(tcasnp, "1.2.840.113549.1.1.11");
    tcasnp->level = level;
    tcasnp++;
    tcasnp->type = ASN_OBJ_ID;
    tcasnp->lth = _write_objid(tcasnp, "2.16.840.1.101.3.4.2.1");
    tcasnp->level = level;
    tcasnp++;
...
void AlgorithmTableDefined(struct AlgorithmTableDefined *mine, ushort level)
    ...
    simple_constructor(&mine->rsadsi_SHA256_WithRSAEncryption, level, ASN_NULL);
    simple_constructor(&mine->sha256, level, ASN_NULL);
