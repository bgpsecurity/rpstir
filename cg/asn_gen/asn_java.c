...
static char *add_list(
    ...
    *i_names,
    ...
    set_sub_default[] =
    "        _set_sub_flag(%s.%s, (short)(AsnStatic.ASN_DEFAULT_FLAG));\n",
...
void jconstruct(
    char *dir,
    char *include_names,
    int include_namesize)
{
    /*
     * Function: Creates class definitions for the things defined an ASN.1
     * file. Outputs: C code written to 'outstr' Procedure: 1. WHILE have a
     * next token Switch on state 2. Case GLOBAL IF reading global returns
     * GLOBAL, return IF have output file, close that Open new file with class 
     * name Write opening material for class Break Case IN_DEFINITION IF token 
     * is not '{' (rerun of table) AND reading definition returns less than 0, 
     * return IF STATE is not GLOBAL, construct definition Break 3. Case
     * IN_ITEM Case SUB_ITEM IF it's a table AND java_table returns 0, return
     * ELSE IF java_item returns zero, return FOR all members Read the item
     * Write the member and save the setup line Write the constructor, the
     * setups and the remainder of the class Return what the last item
     * returned Break Default: Exit with fatal message 4. Search name table
     * for pointer items (starting with '_') that have no parent that's a
     * passthrough, i.e. no definition Make a definition for any found IF
     * there are any defined constants Open a file dirname/DirnameStatic.java
     * Print all the constants there Close the file 
     */
    char *c;
    int did,
        classgeneration;
         ...
         get_token(0, token);)
    {
        ...
        case IN_DEFINITION:    /* got ::= */
            if (*token != '{' && read_definition(-1) < 0)
                return;
            if (state != GLOBAL)
                java_def(&classgeneration, &parenttype, &classtag);
            break;
            /*
             * step 3 
             */
        ...
        case SUB_ITEM:
            if ((flags & ASN_TABLE_FLAG))
            {
                ...
                for (state = IN_ITEM; 1; get_token(0, token))
                {
                    if (read_item(-1, construct) < 0)
                        return;
                    if ((*token == ',' || *token == '}') &&
                        !java_item(classgeneration, parenttype,
                                   (struct name_table *)0, 0, 0))
                        return;
                    if (state != IN_ITEM && state != SUB_ITEM)
                        break;
                }
            break;
         ...
         ntbp++)
    {
        ...
        fprintf(outstr, ptr_opener, c,
                (ptbp->
                 flags & ASN_OF_FLAG) ? "implements AsnArrayInterface" : "",
                &c[1]);
    ...
    {
        for (did = 0, ubp = (struct ub_table *)ub_area.area, eubp =
        ...
        if (did || id_area.next > BUILT_IN_IDS)
        {
            ...
            if (id_area.next > BUILT_IN_IDS)
            {
                for (idp = (struct id_table *)id_area.area, eidp =
                     &idp[id_area.next], idp += BUILT_IN_IDS; idp < eidp;
                     idp++)
...
**/
    ...
    if (ntbp && ntbp->pos >= real_start && (ntbp->flags & ASN_FALSE_FLAG))
    {
        ...
        fprintf(outstr, constructor, classname);
        if (ntbp->max)
            fprintf(outstr, array_boundset, ntbp->min, ntbp->max);
        fprintf(outstr, func_finale);
        fprintf(outstr, class_finale);
        end_definition();
        return;
    ...
    if (curr_pos > real_start &&
        ...
        fprintf(outstr, opener, classname,
                ((ntbp->type & ASN_CONSTRUCTED)
                 || (ntbp->flags & ASN_TABLE_FLAG)) ? deriv : find_class(ntbp->
                                                                         type));
    /*
     * step 2 
     */
    ...
    else                        /* no further definition */
    {
                                      ...
                                      (option & ASN_OF_FLAG)))
        {
            if (*subclass)
            {
                ...
                {
                    cat(itemname,
                        ((*subclass == '_') ? &subclass[1] : subclass));
                    *itemname |= 0x20;
                }
            ...
            if (option == ASN_POINTER_FLAG)
            {
                ...
                if (ntbp->type == ntbp->tag || ntbp->tag == 0xFFFFFFFF)
                    fprintf(outstr, ptr_type_tag_w, find_define(ntbp->type));
                else
                    fprintf(outstr, ptr_tag_xw, find_define(ntbp->tag),
                            find_define(ntbp->type));
            }
            else
            {
                if (subtype >= 0 && !*itemname)
                    cat(itemname, array_w);
                if (max)
                {
                ...
                if (ntbp && ntbp->type < ASN_CONSTRUCTED)
                    subtype = (short)ntbp->type;
                if (*itemname)
                    print_item((*itemname == '_') ? &itemname[1] : itemname,
                               // (subtype > 0 && (!*subclass || (ntbp &&
                               // !(ntbp->flags & ASN_ENUM_FLAG))))?
                               (subtype > 0) ?
                               subtype : 0, (option & ~(ASN_OF_FLAG)), max,
                               min);
                print_delay_list();
                if (subtype >= 0 && *subclass && ntbp && ntbp->max)
                    fprintf(outstr, sub_boundset, itemname, ntbp->min,
                            ntbp->max);
            }
            fprintf(outstr, func_finale);
...
**/
    ...
    while (state == IN_ITEM || state == SUB_ITEM)
    {
        ...
        if (type < 0)
            ...
            c = (uchar *) find_class(type);
        if (*token == ',')
        {
            b = (uchar *) calloc(1,
                                 strlen(table_write) + strlen(numstring) + 16);
            sprintf((char *)b, table_write, array - 1, numstring, array);
        }
        else
        {
            b = (uchar *) calloc(1,
                                 strlen(table_write_last) + strlen(numstring) +
                                 4);
    ...
    if (ntablep->type == ASN_OBJ_ID)
        b = (uchar *) "AsnObjectIdentifier";
    else
        b = (uchar *) "AsnNumeric";
    fprintf(outstr, table_line, b, b, array);   /* print member */
    fprintf(outstr, constructor, classname);
    if (ntablep->type < 0)
        warn(34, ntablep->name);
    else
    {
...
static int java_item(
    int classgeneration,
    long parenttype,
    struct name_table *ntablep,
    int dup_ansr,
    int from_table)
{
...
**/
    ...
    else if ((flags & (ASN_TABLE_FLAG | ASN_DEFINED_FLAG)) != ASN_TABLE_FLAG)
    {
        ...
        if (*defined_by)
            false_subclass = mk_jsubclass(defined_by);
        if (thisdefined > 1)
...
static void add_delay_list(
    char *new_mem)
{
    delay_list = add_list(delay_list, new_mem);
}

static void add_delay_func_list(
    char *new_mem)
{
    ...
    struct name_table *ptbp)
{
    ...
    if (!*tagqp)
        *tagqp = tqp = (struct tagq *)calloc(sizeof(struct tagq), 1);
    else
    {
...
**/
    ...
    if (dup_ansr == ASN_DUPED_FLAG)     /* dup alone */
    {
        if (loctype == ASN_SET)
            c = "AsnSetArray";
        else if (loctype == ASN_SEQUENCE)
            c = "AsnSequenceArray";
        else if (loctype == ASN_BITSTRING)
            c = "AsnBitString";
        else if (loctype == ASN_OBJ_ID)
            c = "AsnObjectIdentifier";
        else if (loctype == ASN_CHOICE)
            c = "AsnChoice";
        else if (loctype == ASN_ANY)
            c = "AsnAny";
        else if (!(c = find_class(loctype)))
            c = "AsnArray";
    }
             ...
             (loctype == ASN_SEQUENCE || loctype == ASN_SET))
    {
        if (dup_ansr == (ASN_OF_FLAG | ASN_DUPED_FLAG)) /* both */
            ...
            cat(cat(derived_from, c), "Of");    /* plain OF */
        c = derived_from;
    }
    else if ((flags & (ASN_TABLE_FLAG | ASN_DEFINED_FLAG)) == ASN_TABLE_FLAG)
    ...
    int asn)
{
    ...
    if (asn)
        fprintf(outstr, import_line, "asn");
}
...
**/
    char *b;
    b = (char *)calloc(1,
                       sizeof(setup_item) + strlen(prevname) + strlen(name) +
                       4);
    sprintf(b, setup_item, prevname, name);
    add_delay_list(b);
    free(b);
    option = set_options(option, name);
    if (loctag < 0 || (loctag >= ASN_NONE && loctag < ASN_NOTYPE))
        loctag = 0;
...
**/
    ...
    /*
     * lth = strlen(any_item) + (2 * strlen(c)) + strlen(itemname); b = (char
     * *)calloc(1, lth + 2); sprintf(b, any_item, c, itemname, c);
     * add_delay_list(b); free(b); 
     */
    fprintf(outstr, any_item, c, itemname, c);
}

static void print_dup(
    int dup,
    long loctype,
    int use_class)
{
    ...
    if (!use_class && !(loctype & ASN_CONSTRUCTED))
        ...
        deriv = classname;
    if ((dup & ASN_DUPED_FLAG))
        fprintf(outstr, dup_func, classname, classname);
