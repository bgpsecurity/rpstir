...
const char asn_pprocx_rcsid[]="$Header: /nfs/sub-rosa/u1/IOS_Project/ASN/Dev/rcs/cmd/asn_gen/asn_pproc.c,v 1.1 1995/01/11 22:43:11 jlowry Exp gardiner $";
...
extern FILE *make_substr(char *);

extern void empty_substr(FILE *, FILE *, char *, int, char *);

static int collect_args(int, char ***), what_type(char *);

static void append_name(char **, char *),
    do_macro_from_string(FILE *, struct macro_table *, char **),
    find_class_item(char *, struct class_table *, struct with_syntax *, char *),
    insert_name(char *, long);

static char *copy_word(char *, char *),
    *extend_buf(char *, char **, int),
    *fill_id_type(char *, struct class_table *, FILE *),
    *scan_known(char *, char *), *scan_must(char *, char *), *skip_word(char *);

struct chain *add_chain(struct chain *chp, size_t siz)
{
while (chp->next) chp = chp->next;
if (!(chp->next = (struct chain *)calloc(siz, 1))) fatal(7, (char *)0);
return chp->next;
}

int add_class_def(int fd)
{
/**
...
if (ctbp && ctbp->name) fatal(17, classname);
...
for (savec = 0; savec != '}'; )
    ...
    get_known(fd, token, "&");
    get_must(fd, token);
    if (!ctbp->item.name) citp = &ctbp->item;
    else citp = (struct class_item *)add_chain((struct chain *)&ctbp->item,
...
if (strcmp(token, with_w)) syntax(token);
...
while (1)
    ...
    get_must(fd, token);
    if (*token == '}') break;
    if (!ctbp->with_syntax.verb) wsxp = &ctbp->with_syntax;
    else wsxp = (struct with_syntax *)add_chain(
        (struct chain *)&ctbp->with_syntax, sizeof(struct with_syntax));
    if (*token == '[')
	{
        wsxp->optional = 1;
	get_must(fd, token);
    ...
    *c++ = ' ';
    ...
    if (wsxp->optional) get_known(fd, token, "]");
    }
return GLOBAL;
}

void add_class_member(struct class_table *ctbp, char *name)
{
struct class_table *nctbp;
if ((nctbp = find_class_entry(name))) fatal(17, name);
...
struct import_table *add_import_item(struct import_table *timtbp,
    char *itemname)
{
/**
...
void add_macro(int fd, char *name)
{
/**
...
**/
int count, braces, mac_size, parens, got_def;
size_t lth, size;
struct macro_table *mtbp;
char *macbuf, *c, **argpp, *b, *d;
...
else mtbp = (struct macro_table *)0;
count = collect_args(fd, &argpp);
if (mtbp) mtbp->arg_count = count;
else free((char *)argpp);
							    /* step 2 */
...
for (braces = parens = got_def = 0, c = macbuf; braces || *token != '}'; )
    ...
    get_must(fd, token);
    count = -1;
    if (*token == ',' || (*token == '{' && !braces)) cat(&token[1], "\n");
    if (*token == '{') braces++;
	...
	for (b = token; *b; b++)
		...
		if (d == &b[lth] && !strncmp(argpp[count], b, lth)) break;
		}
	    if (count >= 0) break;
	    }
	if (*token == '(') parens++;
	if (count < 0 && !parens && !braces && got_def++) break;
	if (*token == ')') parens--;
	}
	...
	c = cat(c, " %s");
	if (mitp->prefix) mitp = (struct macro_item *)add_chain((struct chain *)
	    mitp, sizeof(struct macro_item));
...
if (*token == '}') *token = 0;  /* had terminating braces */
							    /* step 3 */
if (mtbp)
    {
    ...
    if (!*token && *peek_token(fd) == '(')
        {
        if (!(b = c = (char *)calloc((size = 32), 1))) fatal(7, (char *)0);
        get_known(fd, c++, "(");
        *c++ = ' ';
...
static void append_name(char **pp, char *val)
{
char *c, locbuf[80];
size_t lth;
for (c = val; *c > ' '; c++);
...
void class_instance(int fd, FILE *str, struct class_table *ctbp, char *item)
{
/**
...
struct macro_table *mtbp;
int curr_typ, typ, braces, parens, do_it = (!fd || is_imported(item));
char *nouns[2], locbuf[128], *c, *b;
        						/* step 1 */
nouns[0] = nouns[1] = 0;
if (!wsxp->verb) wsxp = wsxp->next;
if (!wsxp) fatal(20, "syntax entry");
							/* step 2 */
for (braces = 1, parens = 0, curr_typ = -2, c = locbuf; braces || parens; )
	...
	if (curr_typ == VERB)
		...
		{
		if (wsxp->table_outp) find_class_item(item, ctbp, wsxp, nouns[1]);
                wsxp = wsxp->next;
		}
            if (!wsxp) fatal(20, locbuf);
	    }
	else
	    {
	    if (*locbuf == '{' && braces == 1)
		...
		curr_typ = what_type(locbuf);
            ...
            if (curr_typ > 0) fill_name(&nouns[curr_typ - TYP_REF], locbuf);
	    }
	curr_typ = typ;
	c = cat(locbuf, token);
	}
    if (*token == '}') braces--;
    else if (*token == ')' && --parens < 0) syntax(token);
    }
							       /* step 3 */
if (do_it)
    {
    if ((wsxp->subject && !nouns[0]) ||
        (wsxp->object && !nouns[1])) fatal(20, (!*nouns)? "Type": "id");
	...
	if (tbep)
	    {
	    if (!tbep->item) fill_name(&tbep->item, item);
	    }
	else if (!do_it) break;     /* did one; have no more */
	else continue;          /* none here; keep trying */
        if (nouns[0]) append_name(&tbep->value, nouns[0]);
        if (nouns[1]) append_name(&tbep->id,    nouns[1]);
	do_it = 0;      /* did one */
	}
    }
end_definition();
...
static int collect_args(int fd, char ***argppp)
{
size_t count;
char **argpp = (char **)0;
struct class_table *ctbp;
...
for (get_must(fd, token), braces = count = 0; *token != '}' || braces;
	...
	(argpp &&  !(argpp = (char **)realloc((char *)argpp, sizeof(char *) *
        (count + 1))))) fatal(7, (char *)0);
...
void collect_ids(int fd, struct class_table *ctbp, FILE *str)
{
/**
...
for (braces = item = 1; braces; )
	...
	{
	if (tbep->item) tbep = (struct table_entry *)add_chain(
    	    (struct chain *)tbep, sizeof(struct table_entry));
...
void collect_id_type(int fd, struct class_table *ctbp, FILE *str)
{
/**
...
2. Call fill_id_type.
**/
char *c, locbuf[128], *tbuf;
int tbuf_size;
struct with_syntax *wsxp = &ctbp->with_syntax;
						    /* step 1 */
if (!wsxp->verb) fatal(20, "syntax table");
if (!ctbp->instance_name) fill_name(&ctbp->instance_name, classname);
c = tbuf = (char *)calloc((tbuf_size = 40), 1);
get_known(fd, token, "&");
get_must(fd, token);        /* membername */
c = cat(cat(c, " &"), token);
while (wsxp && (!wsxp->subject || strcmp(token, wsxp->subject)) &&
    (!wsxp->object || strcmp(token, wsxp->object))) wsxp = wsxp->next;
if (!wsxp) fatal(28, token);
if (*peek_token(fd) == '(')
    {
    get_known(fd, token, "(");
    get_known(fd, token, "{");
...
char *copy_word(char *to, char *from)
{
while (*from > ' ') *to++ = *from++;
*to = 0;
return to;
...
void do_macro(int fd, FILE *str, struct macro_table *mtbp)
{
/**
...
**/
char **argpp, locbuf[80];
int count;
							    /* step 1 */
cat(locbuf, classname);
*locbuf &= ~(0x20);      /* capitalize name */
fprintf(str, "%s ::= ", locbuf);
if (*token != '{') get_must(fd, token);
if (*token != '{') fatal(26, few_w);
...
void do_macro_from_string(FILE *str, struct macro_table *mtbp, char **argpp)
{
/**
...
**/
char *a, *b, *c, *argp, *tbuf, savec, locbuf[80], subfilename[80],
    **locargpp = (char **)0;
int count, arg_num, old_arg_num, tbufsize;
struct macro_item *mitp;
struct class_table *ctbp, *tctbp;
struct macro_table *tmtbp;
FILE *substr = (FILE *)0;
							    /* step 1 */
c = tbuf = (char *)calloc((tbufsize = 50), 1);
for (mitp = &mtbp->item, ctbp = (struct class_table *)0; mitp; mitp = mitp->next)
    {
    ...
    for ( ; *b; b = skip_word(b))
	    ...
	    *tbuf = 0;
	    ...
	    if (*b == '{')
		{
		locargpp = (char **)calloc(sizeof(char *), tmtbp->arg_count);
                for (b = skip_word(b), count = 1, arg_num = old_arg_num = 0;
                    *b && (count > 1 || *b != '}'); b = skip_word(b))
		    {
		    if (*b == '{') count++;
		    else if (*b == '}') count--;
		    else if (*b == '(') count += 20;
			...
			{
                        if (arg_num >= tmtbp->arg_count) fatal(26, many_w);
			for (a = b; *a > ' '; a++);
			locargpp[arg_num] = (char *)calloc((&a[1] - b), 1);
			strncpy(locargpp[arg_num++], b, (a - b));
			}
		    }
		if (arg_num < tmtbp->arg_count) fatal(26, few_w);
		}
	    if (locargpp) argpp = locargpp;
	    do_macro_from_string(substr, tmtbp, argpp);
	    if (mitp->next && mitp->next->prefix && *mitp->next->prefix == '}')
		mitp = mitp->next;
	    }
	else
	    {
...
fprintf(str, "\n\n");
if (substr) empty_substr(str, substr, tbuf, tbufsize, subfilename);
free(tbuf);
if (locargpp) free(locargpp);
*token = 0;
}

int encr_xform(int fd, char *loctoken)
{
/**
Procedure:
...
char *extend_buf(char *buf, char **ebuf, int lth)
{
char *c;
if (!(c = realloc(buf, lth))) fatal(7, (char *)0);
*ebuf += (c - buf);
return c;
}

char *fill_id_type(char *string, struct class_table *ctbp, FILE *str)
{
/**
...
while (wsxp && (!wsxp->subject || strcmp(locbuf, wsxp->subject)) &&
    (!wsxp->object || strcmp(locbuf, wsxp->object))) wsxp = wsxp->next;
if (!wsxp) fatal(28, string);
while (*string && *string <= ' ') string++;
if (*string != '(')
    {
    for(citp = &ctbp->item; citp && strcmp(citp->name, wsxp->object); citp =
        citp->next);
    if (!citp) fatal(28, wsxp->object);
    for (c = citp->predicate; *c && !is_reserved(c); )
	{
	while (*c > ' ') c++;
	while (*c && *c <= ' ') c++;
	}
    if ((*c & 0x20)) fprintf(str, any_w);
    else fprintf(str, citp->predicate);
    return string;
    }
string = scan_known(string, "(");
string = scan_known(string, "{");
...
if ((ctbp->with_syntax.next || wsxp->subject))
    ...
    if (!tbop) tbop = (struct table_out *)add_chain(
    	    (struct chain *)&ctbp->table_out, sizeof(struct table_out));
    if (!tbop->table_name) fill_name(&tbop->table_name, tbname);
    }
if (*locbuf)
    {
    if (!(citp = &ctbp->item)->name) citp = citp->next;
    if (!citp) fatal(28, tbname);
    while (citp && strcmp(citp->name, wsxp->object)) citp = citp->next;
    for (c = citp->predicate; *c > ' '; c++);
    if (!*c) tmp = find_type(citp->predicate);
    else
	{
        *c = 0;
    ...
    if (tmp != ASN_BOOLEAN && tmp != ASN_OCTETSTRING && tmp != ASN_BITSTRING)
	c = "ANY";
    else c = citp->predicate;
...
void fill_name(char **to, char *from)
{
if (*to) free(*to);
if (!(*to = (char *)calloc(strlen(from) + 1, 1))) fatal(7, (char *)0);
cat(*to, from);
}

void fill_table_entry(int fd, int index)
{
/**
...
struct with_syntax *wsxp, tsx;
struct table_entry *ftbp, *xtbp;
								/* step 1 */
...
for (wsxp = &ctbp->with_syntax; wsxp; wsxp = wsxp->next)
    ...
    if (wsxp->subject)
	{
        if ((*(wsxp->subject) & 0x20)) idp = tsx.subject;
	else valp = tsx.subject;
	}
    if (wsxp->object)
	{
        if ((*(wsxp->object) & 0x20)) idp = tsx.object;
	else valp = tsx.object;
	}
    if (idp) fill_name(&ftbp->id, idp);
    if (valp)
	{
	if ((xtbp = ftbp)->value) xtbp = (struct table_entry *)add_chain(
	    (struct chain *)xtbp, sizeof(struct table_entry));
	fill_name(&xtbp->value, valp);
	}
    if (tsx.subject) free(tsx.subject);
    if (tsx.object)  free(tsx.object);
    free(tsx.verb);
    }
							       /* step 3 */
...
struct class_table *find_class_entry(char *name)
{
struct class_table *ctbp;
char *c, savec;
for (c = name; *c && *c != '.'; c++);
...
*c = savec;
if (!ctbp || !ctbp->name) return (struct class_table *)0;
return ctbp;
}

static void find_class_item(char *item, struct class_table *ctbp,
    struct with_syntax *wsxp, char *val)
{
struct class_item *citp;
struct table_entry *tbep;
char *b, *c;
...
if (wsxp->table_outp && (tbep = &wsxp->table_outp->table_entry))
    {
    while (tbep && tbep->item && strcmp(item, tbep->item)) tbep = tbep->next;
    if (tbep && tbep->item && c) append_name(&tbep->value, c);
    }
}

struct macro_table *find_macro(char *name)
{
...
for (mtbp = (struct macro_table *)macro_area.area, emtbp =
    &mtbp[macro_area.next]; mtbp < emtbp && strcmp(name, mtbp->name); mtbp++);
if (mtbp >= emtbp) mtbp = (struct macro_table *)0;
*c = savec;
return mtbp;
}
...
void get_exports(int fd, FILE *str)
{
/**
...
int export_all = 0;
if (!fd) fprintf(str, "%s ", token);
for (*token = 0; get_token(fd, token) && *token != ';' ; )
    {
    if (*token > ' ' && *token != ',')
	{
	if (!strcmp(token, all_w)) export_all = 1;
	else add_name(token, (long)-1, (!fd)? ASN_EXPORT_FLAG: 0);
	}
    if (!fd) fprintf(str, "%s ", token);
    }
if (!fd) fprintf(str, ";\n\n");
if (export_all) return;
...
if (imtbp && imtbp->name)                                   /* step 2 */
    ...
    for (itemp = &imtbp->item; itemp; itemp = itemp->next)
        {
        for (ntbp = bntbp; ntbp <= entbp && strcmp(ntbp->name, itemp->objname);
            ntbp++);
        if (ntbp > entbp)
    	    {
	...
	{
	for (itemp = &imtbp->item; itemp && strcmp(ntbp->name, itemp->objname);
            itemp = itemp->next);
	if (!itemp)
            {
            free(ntbp->name);
...
**/
char *objname = (char *)0;
struct import_table *eimtbp, *timtbp, ximtb ;
...
while (get_must(fd, token) && *token != ';')
	...
	if (!get_token(fd, token)) break; /* error msg later */
	}
    while (*token != ';' && strcmp(token, from_w));
							/* step 2 */
    if (*token == ';' ||
        !get_token(fd, token)) syntax(token);
	...
	if (ximtb.name) timtbp->name = ximtb.name;
	ximtb.name = (char *)0;
...
char *get_obj_id(int fd, char *name, char *basename)
{
/**
...
for (id_string = 0, cat(locname, name); *token != '}'; )
	...
	if (pidp && pidp->name && *pidp->name && (!basename ||
		...
		{
		if (id_string) fatal(13, token);
		fill_name(&id_string, token);
		return id_string;
		}
	    else if (*locbuf >= '0' && *locbuf <= '9') fatal(18, name);
	    else fatal(18, name);
	    }
	}
	...
	get_must(fd, token);
	get_known(fd, locbuf, ")");
	if (strcmp(token, pidp->val)) fatal(17, locname);
	get_must(fd, token);
	}
    if (*token >= 'a' && *token <= 'z') cat(locname, token);
    else *locname = 0;
    }
return id_string;
}

static void insert_name(char *buf, long typ)
{
char *c = cat(buf, find_typestring(typ));
if (typ == ASN_OBJ_ID) c = cat(cat(c, " "), identifier_w);
else if (typ == ASN_BITSTRING || typ == ASN_OCTETSTRING)
    c = cat(cat(c, " "), string_w);
}

int is_imported(char *name)
{
struct import_item *itemp;
...
char *scan_known(char *from, char *kn)
{
while (*from && *from <= ' ') from++;
while (*kn && *kn++ == *from) from++;
if (*kn) syntax(from);
return from;
}

char *scan_must(char *from, char *to)
{
char *c;
for (c = from; *c && *c <= ' '; c++);
if (!*c) syntax(from);
...
void scan_syntax(int fd, struct with_syntax *wsxp, struct with_syntax
    *tsxp)
{
char *c;
int ansr;
size_t lth, siz = strlen(wsxp->verb);
...
do
	...
	{
        if (wsxp->subject) fill_name(&tsxp->subject, token);
	}
	...
	for (c = &token[lth], *c++ = ' '; !strncmp(token, wsxp->verb, lth) &&
            lth < siz; c += (ansr + 1), *c++ = ' ', lth = c - token)
	    {
	    ansr = get_must(fd, c);
	    }
	if (!strcmp(token, wsxp->verb) && lth == siz)
	    {
	    if (wsxp->object) get_must(fd, token);
	    if (*token == '{')
		{
		get_must(fd, token);
...
if (!ansr) syntax(wsxp->verb);
}

void test_paren(int fd, char *buf, char *linebuf, char **linendpp,
char *elinebuf)
{
int parens = 1;
get_must(fd, &buf[2]);
if (buf[2] == '(') parens++;
else if (buf[2] == ')') parens--;
buf[1] = ' ';
...
static int what_type(char *string)
{
/**
...
if ((*string & 0x20) || !strcmp(string, true_w) ||
    ...
    return VAL_REF;
if (find_type(string) != ASN_NOTYPE) return TYP_REF;
