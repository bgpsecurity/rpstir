...
static struct Attribute *find_attr(
    ...
    char *oidp)
{
         ...
         attrp; attrp = (struct Attribute *)next_of(&attrp->self))
    {
        ...
        {
            if (num++ || num_items(&attrp->attrValues.self) != 1)
                return NULL;
            ch_attrp = attrp;
        }
    }
...
{                               // used for manifests alg = 1 for SHA-1; alg = 
    ...
    char *msg = NULL;
    if (cryptCreateContext(&hashContext, CRYPT_UNUSED, alg) < 0)
        msg = "create";
    else if (cryptEncrypt(hashContext, inbufp, bsize) < 0)
        msg = "encrypt 1";
    else if (cryptEncrypt(hashContext, inbufp, 0) < 0)
        msg = "encrypt 2";
    else if (cryptGetAttributeString
             (hashContext, CRYPT_CTXINFO_HASHVALUE, hash, &ansr) < 0)
        msg = "get attribute";
    else if (cryptDestroyContext(hashContext) < 0)
        msg = "destroy";
    else
...
static int check_cert_signature(
    ...
    struct ROA *rp)
{
    ...
    if (num_items(&rp->content.signedData.digestAlgorithms.self) != 1 ||
        diff_objid(&rp->content.signedData.
                   digestAlgorithms.cMSAlgorithmIdentifier.algorithm,
                   id_sha256))
    ...
    struct Certificate *certp =
        (struct Certificate *)member_casn(&rp->content.signedData.certificates.
                                          self, 0);
    if (num_items(&rp->content.signedData.crls.self))
        return ERR_SCM_CRL;
    struct Extension *extp;
    for (extp =
    ...
    if (!(attrp = find_attr(&sigInfop->signedAttrs, id_contentTypeAttr)) ||
        ...
        return ERR_SCM_BADSIGINFO;
    /*
     * skip signing time if (!(attrp = find_attr(&sigInfop->signedAttrs,
     * id_signingTimeAttr))) return ERR_SCM_BADSIGINFO; // make sure it is
     * the right format uchar loctime[30]; int usize, gsize; if ((usize =
     * vsize_casn(&attrp->attrValues.array.signingTime.utcTime)) > 15 ||
     * (gsize = vsize_casn(&attrp->attrValues.array.signingTime.
     * generalizedTime)) > 17) return ERR_SCM_BADSIGINFO; if (usize > 0) {
     * read_casn(&attrp->attrValues.array.signingTime.utcTime, loctime); if
     * (loctime[0] <= '7' && loctime[0] >= '5') return ERR_SCM_BADSIGINFO; }
     * else { read_casn(&attrp->attrValues.array.signingTime.generalizedTime,
     * loctime); if (strncmp((char *)loctime, "2050", 4) < 0) return
     * ERR_SCM_BADSIGINFO; } 
     */
    // check the hash
    memset(hashbuf, 0, 40);
    ...
    if (size_casn(&rp->content.signedData.crls.self) > 0 ||
        ...
        diff_casn_num(&rp->content.signedData.signerInfos.signerInfo.
                      version.self, 3) != 0)
        return ERR_SCM_BADVERS;
    ...
    if (diff_objid
        (&rp->content.signedData.signerInfos.signerInfo.signatureAlgorithm.
         algorithm, id_rsadsi_rsaEncryption))
        return ERR_SCM_INVALSIG;
    ...
    char **argv)
{
    ...
    if (get_casn_file(&roa.self, argv[1], 0) < 0)
        fatal(2, argv[1]);
    if (get_casn_file(&etacert.self, argv[2], 0) < 0)
        fatal(2, argv[2]);
    ...
    if (diff_objid(&roa.content.signedData.encapContentInfo.eContentType,
        ...
        fatal(3, "eContentType");
    struct SignerInfos *signerInfosp = &roa.content.signedData.signerInfos;
    if (num_items(&signerInfosp->self) != 1)
        fatal(3, "number of SignerInfos");
    struct SignerInfo *signerInfop =
        (struct SignerInfo *)member_casn(&signerInfosp->self, 0);
    ...
    if (j >= num)
        fatal(3, "content type attribute");
    for (j = 0; j < num; j++)
    {
        attrp = (struct Attribute *)member_casn(&sigAttrsp->self, j);
        if (!diff_objid(&attrp->attrType, id_messageDigestAttr))
            break;
    }
    if (j >= num)
        fatal(3, "message digest attribute");
    struct Certificate *rtacertp =
        &roa.content.signedData.encapContentInfo.eContent.trustAnchor;

    if (check_cert_signature(rtacertp, rtacertp) < 0)
        fatal(2, "signature");
    if (num_items(&roa.content.signedData.certificates.self) != 1)
        fatal(3, "number of certificates");
    struct Certificate *eecertp =
