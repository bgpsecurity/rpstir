...
void printAddress(struct IPAddressA *addressPrefixp, int tot, char *fill)
  {
  uchar buf[36];
  char text[36];
  ...
  if (buf[0]) 
    ...
    uchar mask = 0xff;
    mask >>= (8 - buf[0]);
    if (*fill == '0') buf[lth - 1] &= ~mask;
    else buf[lth - 1] |= mask;
    };
  int i, j;
  memset(text, 0, 36);
  for (i = 1, j = 0; i < lth; i++, j++)  sprintf(&text[2 * j], "%02x", buf[i]);
  while(j < tot) sprintf(&text[2 * j++], "%s", fill);
  for (i = 0, tot <<= 1; i < tot; )
    {
    printf("%c", text[i++]);
    printf("%c", text[i++]);
...
void printRange(struct IPAddressRangeA *addressRangep, int tot)
  {
...
int main(int argc, char **argv)
  ...
  for (p = &argv[1]; p < &argv[argc]; p++)
    ...
    if ((lth = get_casn_file(&extensions.self, *p, 0)) < 0) fatal(5, *p);
    struct Extension *extp;
    if (!(extp = find_extn(&extensions, id_pe_ipAddrBlock)))
      fatal(4, "IPAddress");
    printf("File %s\n", *p);
    struct IpAddrBlock *ipaddrblockp = &extp->extnValue.ipAddressBlock;
    struct IPAddressFamilyA *ipaddrfamap = (struct IPAddressFamilyA *)member_casn(
      &ipaddrblockp->self, 0);
    uchar fam[2];
    read_casn(&ipaddrfamap->addressFamily, fam);
    printf("%s\n", (fam[1] == 1? "v4": "v6"));
    struct IPAddressChoiceA *ipaddrchoiceap = &ipaddrfamap->ipAddressChoice;
    ...
    for (ipaddrorrangeap = (struct IPAddressOrRangeA *)member_casn(
      &addrsOrRangesp->self, 0); ipaddrorrangeap;
      ipaddrorrangeap = (struct IPAddressOrRangeA *)next_of(
      &ipaddrorrangeap->self))
      {
      ...
      else printRange(&ipaddrorrangeap->addressRange, 4);
      }
    ...
    for (ipaddrorrangeap = (struct IPAddressOrRangeA *)member_casn(
      &addrsOrRangesp->self, 0); ipaddrorrangeap;
      ipaddrorrangeap = (struct IPAddressOrRangeA *)
      next_of(&ipaddrorrangeap->self))
      {
      if (vsize_casn(&ipaddrorrangeap->addressPrefix) > 0)
        {
      ...
      else printRange(&ipaddrorrangeap->addressRange, 10);
      }
