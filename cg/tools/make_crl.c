...
/*
  ...
  setTime(&crltbsp->lastUpdate.generalTime, (ulong)now);
  now += (60 * 24 * 3600);  // 60 days
  setTime(&crltbsp->nextUpdate.generalTime, (ulong)now);
*/
  clear_casn(&crltbsp->lastUpdate.self);
  write_casn(&crltbsp->lastUpdate.generalTime, (uchar *)"20000101000000Z", 15);
  clear_casn(&crltbsp->nextUpdate.self);
  write_casn(&crltbsp->nextUpdate.generalTime, (uchar *)"20300101000000Z", 15);
  ...
  for (numcerts = 0; fgets(certbuf, 40, stdin) && certbuf[0] > ' '; )
    ...
    char datebuf[16];
    sprintf(datebuf, "%d%02d%02d000000Z", yr, mo, da);
    write_casn(&crlentryp->revocationDate.generalTime, (uchar *)datebuf, 15);
    }
