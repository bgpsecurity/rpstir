...
static int roa2prefix(char **prefixpp, struct ROAIPAddress *roaipp, int family)
  {
...
static int read_family(char **fampp, struct ROAIPAddressFamily *roafamp)
  {
  uchar ub[8];
  ...
  if (ub[1] == 2) buf[3] = '6';
  c = &buf[5];
  int i, num = num_items(&roafamp->addresses.self); 
  for (i = 0; i < num; i++)
    {
    struct ROAIPAddress *roaipp = (struct ROAIPAddress *)member_casn(&roafamp->addresses.self,
      i);
    if (!roaipp) fatal(1, buf); 
...
int main(int argc, char **argv)
  ...
  if (argc < 2)
    {
    fprintf(stderr, "Parameters are: ROA file name and optional output file name\n");
    exit(0);
    }
  FILE *str = stdout;
  if (argc == 3 && (str = fopen(argv[2], "w")) == 0) fatal(2, argv[2]);
  struct ROA roa;
  ROA(&roa, (ushort)0);
  ...
  int i, num = num_items(&roablockp->self);
  for (i = 0; i < num; i++)
    {
    int ansr;
    struct ROAIPAddressFamily *roafamp = (struct ROAIPAddressFamily *)member_casn(
      &roablockp->self, i);
    char *a;
    if ((ansr = read_family(&a, roafamp)) 
 < 0) fatal(5, (char *)i);
    fprintf(str, a);
    free(a);
    }
