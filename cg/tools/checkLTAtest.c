...
int main(int argc, char **argv)
  ...
  if (argc == 0 || argc < 3) fatal(1, (char *)0);
  char **p;
  for (p = &argv[2]; p < &argv[argc]; p++)
    ...
    if (get_casn_file(&extensions.self, filename, 0) < 0) fatal(5, filename);
    if ((lth = get_casn_file(&cert.self, *p, 0)) < 0) fatal(5, *p);
    struct Extension *extp;
    if (!(extp = find_extn(&cert.toBeSigned.extensions, id_pe_ipAddrBlock)))
      fatal(4, "IPAddress");
    struct Extension *nextp = (struct Extension *)member_casn(&extensions.self, 0);
    if (diff_casn(&nextp->self, &extp->self)) fatal(6, *p);
    if (!(extp = find_extn(&cert.toBeSigned.extensions, 
      id_pe_autonomousSysNum))) fatal(4, "AS number");
