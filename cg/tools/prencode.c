...
#ifndef lint
const char prencode_rcsid[] =
    "$Header: /nfs/flippet/u1/Mosaic/Dev/rcs/lib/asn1utils/prencode.c,v 1.2 1993/09/02 13:56:37 dpn Exp $";
#endif
...
int convto6b(
    src,
    dest)                       /* convert 8 printables to 6 8-bit bytes */
     char src[],
         dest[];
{
    int i,
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        if (c == ENCNULL)
        {
            ...
            case 0:
                rv = 0;
                break;          /* all padding (bad case) */
            case 1:
                rv = 0;
                break;          /* 1 data, 7 pad (bad case) */
            case 2:
                rv = 1;
                break;          /* 2 data, 6 pad */
            case 3:
        ...
        {                       /* 
                                 * printf ("Can't find bit value for character 
                                 * %c (hex %x)\n", c, (unsigned) c); */
        }
#else
