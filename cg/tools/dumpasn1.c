...
static const char *configPaths[] = {
    ...
    /* special for convenience with the RPKI work */
    "$HOME/apki/trunk/cg/tools",
...
static void displayString( FILE *inFile, long length, int level,
    ...
    if( length > 384 && !printAllData )
	    ...
	    // print "mm/dd" not "dd/mm" (in US-order not Euro-order)
            fprintf( output, " %c%c/%c%c/", timeStrPtr[ 2 ], timeStrPtr[ 3 ],
