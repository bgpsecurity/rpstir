    ...
    char **argv)
{
    ...
    if (!(aextp = find_extension(&ETAcert, id_subjectKeyIdentifier)) ||
        ...
        diff_casn(&aextp->extnValue.subjectKeyIdentifier,
                  &sextp->extnValue.authKeyId.keyIdentifier))
        fatal(3, "key identifier");
    if (diff_casn(&ETAcert.toBeSigned.subject.self,
                  &EEcert.toBeSigned.issuer.self))
        fatal(3, "name");
    ...
    struct CMSAlgorithmIdentifier *algp =
        (struct CMSAlgorithmIdentifier *)inject_casn(&signedDatap->
    ...
    if (!
        ...
        || copy_casn(&certp->self, &EEcert.self) < 0)
        fatal(2, "EE certificate");
    // fill in SignerInfo
    struct SignerInfo *signerInfop =
        (struct SignerInfo *)inject_casn(&signedDatap->signerInfos.self, 0);
    if (!signerInfop)
        fatal(2, "SignerInfo");
