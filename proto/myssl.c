...
#include "crlv2.h"
     ...
     * that will be acceptable to the DB. The return value is allocated
     * memory.  The time string can be either UTC or GENERALIZED. If
     * only_gentime is false, UTC is used for dates <= 2049 and GENERALIZED
     * is used for dates after >= 2050. Otherwise, GENERALIZED is use for all 
     * dates.  The UTC format takes the form YYMMDDHHMMSST, where each of
     * the fields is as follows: if YY <= 49 the year is 2000+YY otherwise it 
     * is 1900+YY 1 <= MM <= 12 1 <= DD <= 31 0 <= HH <= 24 0 <= MM <= 60
     * 0 <= SS <= 60 (seconds field is optional) T, is present and == Z
     * indicates GMT  The GENERALIZED format takes the form YYYYMMDDHHMMSST,
     * where the year is given in the full four digit form, and all other
     * fields are the same. Note that seconds can be given as either SS or
     * SS.S.  Both fields can have an optional suffix of the form +HHMM or
     * -HHMM.  On success, *stap ("status pointer") is set to 0.  On failure, 
    ...
    int only_gentime) 
{
        ...
        
    {
        ...
        // next, determine how many characters there are before the tz
        // indicator
        ptr = strchr(bef, 'Z');
        ...
        
    {
            ...
            sscanf(bef, "%2d%2d%2d%2d%2d%c", &year, &mon, &day, &hour, &min,
                   &tz);
            ...
            
        {
            ...
            sscanf(bef, "%2d%2d%2d%2d%2d%2d%c", &year, &mon, &day, &hour,
                   &min, &sec, &tz);
            ...
            
        {
            ...
            sscanf(bef, "%4d%2d%2d%2d%2d%2d%c", &year, &mon, &day, &hour,
                   &min, &sec, &tz);
            ...
            
        {
            ...
            sscanf(bef, "%4d%2d%2d%2d%2d%2d.%1d%c", &year, &mon, &day, &hour,
                   &min, &sec, &msec, &tz);
             ...
             */ 
    {
        ...
        // we should adjust the time if there is a suffix, but currently we
        // don't
...
{cf_get_ipb, 0, NID_sbgp_ipAddrBlock, 0, 1}, {cf_get_flags, 0, NID_basic_constraints, 0, 0} 
    ...
    /*
     *  Given an X509V3 extension tag, this function returns the
     * corresponding extension validator. 
     */ 
...
{cf_get_subject, CF_FIELD_SUBJECT, 1}, {cf_get_issuer, CF_FIELD_ISSUER, 1}, {cf_get_sn, CF_FIELD_SN, 1}, {cf_get_from, CF_FIELD_FROM, 1}, {cf_get_to, CF_FIELD_TO, 1}, {cf_get_sig, CF_FIELD_SIGNATURE, 1}, {NULL, 0, 0}  /* terminator 
    ...
    /*
     *  This function can operate in two ways.  If "fname" and "fullname" are 
     * both given, then it opens a certificate from a file and extracts all
     * the fields from it.  If "xp" points to an already available
     * certificate, then it just manipulates that. This function does not
     * touch the DB at all, it just manipulates the certificate.  On
     * success this function returns a pointer to allocated memory containing
     * all the indicated fields (except the "dirid" field) and sets stap to 0, 
     * and x509stap to 1.  Note carefully that this function does NOT set all 
     * the fields in the cf. In particular, it is the responsibility of the
     * caller to set the dirid field.  This field requires DB access and are
     * therefore is not part of this function.  On failure this function
     * returns NULL and sets stap to a negative error code. If an X509 error
     * occurred, x509stap is set to that error. 
     */ 
    cert_fields * cert2fields(char *fname,
                               char *fullname,
                               int typ,
                               X509 ** xp,
                               ...
                               int *x509stap) 
{
        ...
        
    {
        ...
        if (fname != NULL && fname[0] != 0 && fullname != NULL
            && fullname[0] != 0)
        ...
        
    {
        ...
        // get all the non-extension fields; if a field cannot be gotten and
        // its
        // needed, that is a fatal error. Note that these validators are
        // assumed
        ...
        
    {
                ...
                ASN1_item_d2i(NULL, &udat, ex->value->length,
    ...
    int tag) 
{
    ...
    /*
     *  This function can operate in two ways.  If "fname" and "fullname" are 
     * both given, then it opens a CRL from a file and extracts all the
     * fields from it.  If "xp" points to an already available CRL, then it
     * just manipulates that. This function does not touch the DB at all, it
     * just manipulates the CRL.  On success this function returns a
     * pointer to allocated memory containing all the indicated fields
     * (except the "dirid" field) and sets stap to 0, and crlstap to 1. 
     * Note carefully that this function does NOT set all the fields in the
     * crf. In particular, it is the responsibility of the caller to set the
     * dirid field.  This field requires DB access and are therefore is not
     * part of this function.  On failure this function returns NULL and
     * sets stap to a negative error code. If an X509 error occurred, crlstap 
     * is set to that error. 
     */ 
                            ...
                            void *oidtestp)
{
        ...
        
    {
        ...
        // get all the non-extension fields; if a field cannot be gotten and
        // its
        // needed, that is a fatal error. Note also that these are assumed to
        // be
        // in linear order.
        ...
        
    {
        ...
        // get flags, snlen and snlist; note that snlen is the count in
        // BIGINTs
        cf->flags = 0;
        ...
        
    {
            ...
            
        {
                ...
                
            {
                     ...
                     && !memcmp(ex->object->data, goodoidp->oid,
                                 goodoidp->lth))
                    break;
    ...
    {
        for (ui = 0; ui < sizeof(crxvalidators) / sizeof(crfx_validator);
              ui++)
    ...
    X509 * x) 
{
         ...
         * X509_get_ext_d2i() allows it to return the extensions we are
         * looking for, instead of returning NULL, in certain error cases.
        ...
        if ((pci = X509_get_ext_d2i(x, NID_proxyCertInfo, &crit, NULL)))
    {
             ...
             || X509_get_ext_by_NID(x, NID_subject_alt_name,
    ...
    int ct) 
{
         ...
         * authority), EE_CERT (end entity), or TA_CERT (trust anchor) as
         * definied in the X509 Profile for resource certificates. 
         */ 
        switch (ct)
    {
    ...
    struct Certificate *certp) 
{
    ...
    if (extp)
    {
            ...
            vsize_casn(&certp->toBeSigned.
             ...
             (&certp->toBeSigned.subjectPublicKeyInfo.subjectPublicKey,
              pub_key_infp) != key_info_len)
        {
            ...
            // Subject public key info is a BIT STRING, so the first octet is
            // the
    ...
     for (i = 0; i < X509_get_ext_count(x); i++)
    {
        ...
         if (ex_nid == NID_authority_key_identifier)
        {
                ...
                if (!akid->keyid)
            {
                         ...
                         "[aki] key identifier sub field not present");
    ...
     for (i = 0; i < X509_get_ext_count(x); i++)
    {
        ...
         if (ex_nid == NID_key_usage)
        {
            ...
            if (!X509_EXTENSION_get_critical(ex))
            {
                ...
                /*
                 * I don't like that I'm depending upon other OpenSSL
                 * components for the populating of a parent structure for
                 * testing this, but running out of time and there's no
                 * KEY_USAGE_st and I don't have the time to parse the
                 * ASN1_BIT_STRING (probably trivial but don't know it
                 * yet...). Check asn1/asn1.h for struct asn1_string_st,
                 * x509v3/v3_bitst.c, and get the ASN1_BIT_STRING via
                 * usage=X509_get_ext_d2i(x, NID_key_usage, NULL, NULL) if we 
                 * end up doing this correctly. 
                 */ 
                /*
                 * TODO: possibly replace this function. Notes from Charlie
                 * Oct 2, 2011: In the file
                 * /home/gardiner/cwgrpki/trunk/proto/myssl.c around line 1271 
                 * there is code to check the usage bits.  It requires the
                 * variable "ct", which is available in rescerrt_profile_chk, 
                 * and a pointer to the extension, which should be obtainable 
                 * from the Certificate structure in rescert_profile_chk(). 
                 */ 
        }
    ...
    int ct) 
{
        ...
        
    {
            ...
            
        {
              ...
              i < sk_GENERAL_NAME_num(dist_st->distpoint->name.fullname); i++)
    ...
    int ct) 
{
    ...
    else if (info_flag > 1)
    {
         ...
         * rcynic shows how to get the URI out of the aia... so lifting his
         * teachings.  Though he should be using strncasecmp rather than
         * strncmp as I don't think there are any specifications requiring
    ...
    struct Certificate *certp)
{
    ...
    else if (sinfo_flag > 1)
    {
         ...
         * critical, and there was only one instance of it.  Rob's code from 
         * rcynic shows how to get the URI out of the sia... so lifting his
         * teachings.  Though he should be using strncasecmp rather than
         * strncmp as I don't think there are any specifications requiring
         * the URI to be case sensitive. Additionally, there were no checks
         * in his code to make sure that the RSYNC URI MUST use a trailing
         * '/' in the URI.  
         */ 
    ...
    struct Certificate *certp)
{
        ...
        get_extension(certp, id_pe_ipAddrBlock, &ext_count);
    ...
    struct Certificate *certp)
{
        ...
        get_extension(certp, id_pe_autonomousSysNum, &ext_count);
    ...
    else
    {
        ...
        // Should we make sure there's a non-empty set of AS resources if the
        // AS
        // extension is present and not inherit?
        if (size_casn
             ((struct casn *)&extp->extnValue.autonomousSysNum.rdi.self))
    {
    ...
    int *b) 
{
     ...
     * the Resource Certificate Profile sepecification  This function is
     * used to catch extensions that might be marked critical that we were
     * not expecting. You should only pass this criticals.  Criticals in
     * Resource Certificate Profile: Basic Constraints Key Usage
     * Certificate Policies (NOT Subject Alt Name, which is going to be
    ...
    X509_EXTENSION * ex) 
{
         ...
         * those which are used by the verify process. If an extension is
         * critical and doesn't appear in this list then the verify process
         * will normally reject the certificate. The list must be kept in
         * numerical order because it will be searched using bsearch. 
         */ 
           ...
           ((char *)&ex_nid, (char *)supported_nids,
    ...
    struct RDNSequence *rdnseqp) 
{
        ...
        
    {
            ...
            // RFC 6487 limits the total number of attributes, not the
                ...
                
            {
                    ...
                    
                {
                        ...
                        
                    {
                                 ...
                                 "CommonName not printableString");
    ...
    struct Certificate *certp)
{
    ...
     if (strncmp(outer_sig_alg_oidp, id_sha_256WithRSAEncryption, length) !=
           0)
    {
    ...
     if (strncmp(inner_sig_alg_oidp, id_sha_256WithRSAEncryption, length) !=
           0)
    {
    ...
    if (length <= 0)
    {
         ...
         (&certp->toBeSigned.subjectPublicKeyInfo.algorithm.algorithm,
    ...
    if (bytes_read < 0)
    {
        ...
        // Subject public key modulus must be (SUBJ_PUBKEY_MODULUS_SZ *
        // 8)-bits.
    ...
    struct Certificate *certp)
{
    ...
    if (bytes_to_read > SER_NUM_MAX_SZ)
    {
        ...
        readvsize_casn(&certp->toBeSigned.serialNumber, &sernump);
    ...
    struct Certificate *certp)
{
         ...
         (&certp->toBeSigned.validity.notBefore.self,
    ...
    struct Certificate *certp)
{
    ...
    /*
     *  Perform all checks from http://tools.ietf.org/html/rfc6487 that can
     * be done on a single file. 
     */ 
    ...
    struct CertificateRevocationList *crlp) 
{
         ...
         (&crlp->toBeSigned.signature.algorithm,
    ...
    struct CertificateRevocationList *crlp) 
{
        ...
        diff_casn_time(&crltbsp->lastUpdate.self, &crltbsp->nextUpdate.self);
        ...
        
    {
            ...
            // ERR_SCM_INVALDT; }
            log_msg(LOG_WARNING,
                    "thisUpdate %" PRId64 " seconds in the future",
                    secsInFuture);
