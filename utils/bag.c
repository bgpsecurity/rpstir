...
struct _Bag {
    ...
    bitmap_entry_t *used;       // bitmap of which pointers in entries are
                                // filled in
    bool last_realloc_automatic;        // whether the last realloc was
                                        // automatic or manual (Bag_reserve)
...
inline static size_t bitmap_size(
    size_t num_entries)
{
...
inline static size_t _bitmap_index(
    size_t index)
{
    return index / (8 * sizeof(bitmap_entry_t));
}

inline static bitmap_entry_t _bitmap_mask(
    size_t index)
{
    return (1 << (index % (8 * sizeof(bitmap_entry_t))));
}

inline static bool bitmap_get(
...
inline static void bitmap_clear(
    bitmap_entry_t * bitmap,
    size_t index)
{
    assert(bitmap != NULL);
    ...
    size_t * track_index)
{
    ...
    if (num_entries < bag->allocated_size / 4
        && bag->allocated_size > BAG_ALLOC_START)
    {
        size_t shrink_to = bag->allocated_size / 2;
        if (shrink_to < BAG_ALLOC_START)
            shrink_to = BAG_ALLOC_START;
        ...
        bitmap_entry_t *new_used =
            calloc(bitmap_size(shrink_to), sizeof(bitmap_entry_t));
        if (new_used == NULL)
        {
            free(new_entries);
            BAG_INVARIANTS(bag);
    ...
    else if (num_entries > bag->allocated_size)
    {
        ...
        if (bitmap_size(grow_to) > bitmap_size(bag->allocated_size))
        {
            bitmap_entry_t *new_used =
                realloc(bag->used,
                        bitmap_size(grow_to) * sizeof(bitmap_entry_t));
            if (new_used == NULL)
            {
                BAG_INVARIANTS(bag);
                return false;
            ...
            else
            {
                ...
                memset((void *)(bag->used + bitmap_size(bag->allocated_size)),
                       0,
                       sizeof(bitmap_entry_t) * (bitmap_size(grow_to) -
                                                 bitmap_size(bag->
                                                             allocated_size)));
            }
        }
    ...
    size_t num_entries)
{
    ...
    return Bag_unlock(bag) && ret;      // order of the operands to '&&'
                                        // matters here
}
    ...
    void *data)
{
    ...
    {
        // return value of unlock is ignored because there's nothing good to
        // do with it
        Bag_unlock(bag);
        return false;
    }
...
bool Bag_start_iteration(
    Bag * bag)
{
    return Bag_wrlock(bag);
}

bool Bag_start_const_iteration(
    Bag * bag)
{
    return Bag_rdlock(bag);
}
...
static inline size_t _Bag_iterator_to_index(
    Bag const *bag,
    Bag_const_iterator iterator)
{
    ...
    assert(iterator <
           (Bag_const_iterator) (bag->entries + bag->allocated_size));

    size_t index =
        (void const *const *)iterator - (void const *const *)bag->entries;
...
void *Bag_get(
    ...
    Bag_iterator iterator)
{
BAG_GET_BODY(Bag_iterator, void *)} const void *Bag_const_get(
    Bag const *bag,
    Bag_const_iterator iterator)
{
BAG_GET_BODY(Bag_const_iterator, const void *)}
#undef BAG_GET_BODY
