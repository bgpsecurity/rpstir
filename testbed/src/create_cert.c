...
int use_parent_cert(void *cert, void *val); // filename of parent cert
int use_parent_keyfile(void *cert, void *val); // filename of parent keyfile
int use_subject_keyfile(void *cert, void *val); // filename of subject keyfile
int write_serialNum(void *cert, void *val); 
int write_issuer_name(void *cert, void *val); 
int write_subject_name(void *cert, void *val);
int write_notBefore_time(void *cert, void *val);
int write_notAfter_time(void *cert, void *val);
int write_cert_pubkey(void *cert, void *val);
int write_cert_ski(void *cert, void *val);
int write_cert_aki(void *cert, void *val);
int write_cert_crldp(void *cert, void *val);
int write_cert_sia(void *cert, void *val);
int write_cert_aia(void *cert, void *val);
int write_cert_ipv4(void *cert, void *val);
int write_cert_ipv6(void *cert, void *val);
int write_cert_asnums(void *cert, void *val);
int write_cert_sig(void *cert, void *val);
...
struct object_field cert_field_table[] = 
  {
    ...
    {"serial",INTEGER,NULL, OPTIONAL, write_serialNum},// serial number
    {"issuer", TEXT, NULL, OPTIONAL, write_issuer_name},  // issuers name
    {"subject", TEXT, NULL, OPTIONAL, write_subject_name},  // subject name
...
int use_parent_cert(void *cert, void *val)
{
  ...
  struct Extension *iextp, *cextp;

  // if not parent cert or we are self signed (and shouldn't have a parent cert)
...
int write_default_fields(struct Certificate *certp)
{
  ...
  if (eecert)
	...
	cextp = makeExtension(&ctftbsp->extensions, id_basicConstraints);
      write_casn_num(&cextp->critical, 1);
      //      write_casn(&cextp->extnValue.basicConstraints.self, (uchar *)"", 0); 
 ...
 */
int use_subject_keyfile(void *cert, void *val)
{
  struct Certificate *certp = (struct Certificate *)cert;
  struct CertificateToBeSigned *ctftbsp = &certp->toBeSigned;
 ...
 */
int add_cn(struct RelativeDistinguishedName *rdnp, char *namep, int len)
{
 ...
 */
int add_sn(struct RelativeDistinguishedName *rdnp, char *namep, int len)
{
...
int write_notBefore_time(void *cert, void *val)
{
  struct CertificateToBeSigned *tbsp=&((struct Certificate *)cert)->toBeSigned;
  int len, ret;
  int utclen = 13;
  int glen = 15;  
...
int write_sig(struct Certificate *cert, char *val)
{
  struct CertificateToBeSigned *tbsp=&((struct Certificate *)cert)->toBeSigned;
  int str_len, sig_len;
  char *str_sig = val;
...
int write_key_identifier(struct Certificate *cert, char *id, char *val)
{
  struct CertificateToBeSigned *tbsp=&((struct Certificate *)cert)->toBeSigned;
  int str_len, ki_len;
  char *str_ki = val;
  ...
  if (strncmp(id,id_subjectKeyIdentifier, strlen(id_subjectKeyIdentifier)) == 0)
    {
    ...
    {
      write_casn(&extp->extnValue.authKeyId.keyIdentifier, ki, bytes_written);
      free(ki);
    }
  else
    return -1;
...
int write_cert_crldp(void *cert, void *val)
{
  ...
  while (ptr != NULL)
      ...
      else
	{
      ...
      distp = (struct DistributionPoint *)
        inject_casn(&extp->extnValue.cRLDistributionPoints.self, numpts++);
      if (!distp)
	return -1;
      gennamep = (struct GeneralName *) inject_casn(
...
int write_cert_sia(void *cert, void *val)
{
  ...
  while (ptr != NULL)
      ...
      else
	{
      ...
      accdsp = (struct AccessDescription *)inject_casn(&siap->self, numpts++);
      if (!accdsp)
	return -1;
 ...
 */
int write_cert_addrs(void *cert, void *val, int type)
{
  struct CertificateToBeSigned *tbsp=&((struct Certificate *)cert)->toBeSigned;
  struct Extension *extp;
  struct Extensions *extsp = &tbsp->extensions;
  ...
  if (type == IPv6)
    famp = (struct IPAddressFamilyA *)inject_casn(
			  &extp->extnValue.ipAddressBlock.self, 
...
void clear_cert(struct Certificate *certp)
{
...
void setSelfSigned(struct object_field *table)
{
  char *val;
  int val_type;
  ...
  if (get_table_value("selfsigned", table, &val, &val_type) >= 0)
    {
      if ( (val != NULL) &&
	   (strncasecmp(val, "true", strlen("true")) == 0))
	{
	  selfSigned = 1;
	  eecert = 0;
	}
...
int setEEorCA(struct object_field *table)
{
  char *val;
  int val_type;
 ...
 */
int create_cert(struct object_field *table)
{
  ...
  if (val != NULL) // input signature
    ...
    { // have to sign it, get key from subject keyfile if selfsigned else parents
      if (selfSigned)
