    ...
    for (; generation >= -1; generation--)
    {
        for (ntbp = (struct name_table *)name_area.area; ntbp < entbp; ntbp++)
        {
            ...
            if (*ntbp->name == '_')
            {
                ...
                {
                    if (((dup_ansr =
                          test_dup(ntbp->name, &tmp)) & ASN_DUPED_FLAG))
                        c = "AsnArrayOfPtrs";
                    else
                        c = "AsnPtr";
                       ...
                       ntbp->tag < ASN_CONSTRUCTED)))
            {
                ...
                if (curr_ntbp->type != -1 && curr_ntbp->type < ASN_CONSTRUCTED
                    ...
                    strcpy(itemname, find_class(curr_ntbp->type));      // class 
                                                                        // names 
                                                                        // <
                                                                        // ASN_BSIZE
                ...
                if ((*dup_ansrp & ASN_OF_FLAG))
                {
                    ...
                    if (subtype >= 0)
                    {
                        if (!*subclass)
                        {
                            ...
                            if (subtype < ASN_CHOICE)   // don't print
                                                        // member_func
                                *dup_ansrp &= ~(ASN_OF_FLAG);   // unless
                                                                // defined_by
                        }
                        if (*defined_by)
    ...
    else
    {
        ...
        else
        {
            ...
            for (parentp = &ntablep->parent; parentp; parentp = parentp->next)
            {
                ...
                else if (ptbp->type == ASN_OBJ_ID)
                    fprintf(outstr, assign_char);
...
**/
    ...
    if (dup_ansr == ASN_DUPED_FLAG)
        c = derived_dup(loctype);
    else if ((dup_ansr & ~ASN_POINTER_FLAG) &&
             (loctype == ASN_SEQUENCE || loctype == ASN_SET))
    {
        if (strlen(c) + 14 >= sizeof(derived_from))
            fatal(10, "derived_from");
        if (dup_ansr == (ASN_OF_FLAG | ASN_DUPED_FLAG)) /* both */
            ...
            strcat(strcpy(derived_from, c), "Of");      /* plain OF */
        c = derived_from;
    }
    return c;
}
    ...
    int mode)
{
    ...
    if ((dup_ansr & ASN_OF_FLAG))
        fprintf(outstr, member_func, subclass);
    if ((dup_ansr & ASN_DUPED_FLAG))
        fprintf(outstr, index_op, classname);
    if (mode || type < 0 || *subclass)
    {
        if ((flags & ASN_ENUM_FLAG) && !*subclass)
        {
            if (find_name(classname)->type == ASN_OBJ_ID)
                ...
                fprintf(outstr, num_assigner, classname);
        }
        fprintf(outstr, assigner, classname, classname);
    }
    fprintf(outstr, constructor, classname);
