    ...
    while (buf != NULL)
    {
        ...
        if (read_hex_val(hash, strlen(hash), &hashBits[1]) > 0)
        {
            if (fileName != NULL && hashBits != NULL)
            {
                ...
                if (!
                    (fahp =
                     (struct FileAndHash *)inject_casn(&manp->fileList.self,
                                                       num)))
                    warn(3, "fileList");
    ...
    struct object_field *table)
{
    ...
    int i = 0;
    // Populate the manifest information for everything that has a function
