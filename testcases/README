...
are either CA certificxfates or end-entity certificates.  CA certificates have 
...
3. ROAs
    ...
    ROAs are made with the tool make_test_roa, which takes only one parameter, 
the ROA name.  The name of the ROA consists of "R" followed by digits and
".roa".  The ROA R123.roa is the third ROA produced by CA C12 and signed by
end entity C12R3, e.g.
