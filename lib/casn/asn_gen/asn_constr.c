...
_flags |= ASN_POINTER_FLAG;\n",
    ...
    data_init[] = "%s = 0;\n",
    table_opener[] = "AsnObj *objp = objid;\n\
_tag = _type = %s;\n\
...
_flags |= ASN_POINTER_FLAG;\n",
    ...
    set_sub_default[] = "_set_sub_flag(&%s.%s, (ushort)(ASN_DEFAULT_FLAG));\n",
    ...
    dup_func[] = "AsnObj *%s::_dup()\n\
{\n\
%s *objp = new %s;\n\
_set_pointers(objp);\n\
return objp;\n\
}\n\n",
    index_op[] = "%s& %s::operator[](int index) const\n\
{\n\
...
**/
    ...
    if ((flags & ASN_TABLE_FLAG) && thisdefined > 0)
    {
        ...
        if ((ntbp->flags & ASN_OF_FLAG))
            fprintf(outstr, member_func, classname, ntbp->name, classname);
    }
    else
        c = classname;
    did = test_dup(c, &tmp);
    if (type >= 0 && type < ASN_CONSTRUCTED && !(flags & ASN_ENUM_FLAG))
        did &= ~(ASN_DUPED_FLAG);
    if ((did & ~(ASN_OF_FLAG)) && curr_pos > real_start)
        print_of(did, tmp);
    if (curr_pos > real_start &&
         ...
         *lo_end || constraint_area.next || type >= ASN_CHOICE))
    {
        ...
        if ((option & ASN_OF_FLAG) && *subclass)
        {
            if (subtype < 0)
            {
                get_subtype();
                if (subtype < 0)
                    fprintf(outstr, member_func, subclass, classname,
                            subclass);
                subtype = -1;
            }
        }
        ...
        *itemname |= 0x20;
        if ((option & ASN_ENUM_FLAG))
        {
            ...
            else
            {
                ...
                {
                    if ((size = dot_to_oct_objid(&c, sub_val)) < 0)
                        syntax(token);
                }
                else
                    size = strlen((c = sub_val));
        ...
        print_item(itemname, prevname,
                   (tag_tb < ASN_APPL_SPEC
                    && !*defined_by) ? 0 : tag_tb, (option & ~ASN_TABLE_FLAG),
                   max, min);
        ...
        if ((flags & ASN_TABLE_FLAG))
        {
            ...
            else
            {
                     ...
                     parentp; parentp = parentp->next)
                {
                    ...
                    fprintf(outstr, derived_table, ptbp->name, ptbp->name,
                            path, strlen(path) + 1);
                }
                flags |= ASN_DEFINED_FLAG;
                thisdefined = 1;
                ntablep = find_name(classname);
                if (strlen(classname) > ASN_BSIZE - 8)
                    fatal(10, classname);
    ...
    if (*c)
        for (c++; *c; c++)      /* now do next fields */
        {
            ...
            if (!val)
                ...
                for (tmp = val, siz = 0; tmp; siz++)
                    tmp >>= 7;
            for (a = &top[i], i += siz, tmp = val, b = &a[siz]; siz--;
                 val >>= 7)
                *(--b) =
    ...
    while (*c)
    {
        ...
        {                       // 
            ...
            if (*c >= '0' && *c < '3')
                ...
                fprintf(outstr, "!(AsnObj *)this->diff((AsnObj *)&%s)", c);
        }
        else
        {
            if (*c == '_')
...
**/
    if (!prevname || !*prevname)
        fprintf(outstr, first_item, name);
    else
        fprintf(outstr, later_item, prevname, name);
    option = set_options(option, name);
    if (loctag < 0 || (loctag >= ASN_NONE && loctag < ASN_NOTYPE))
        loctag = 0;
    if (loctag > 0 && loctag < ASN_APPL_SPEC)
        fprintf(outstr, sub_class_w, find_define(loctag));
    else
        fprintf(outstr, sub_tag_xw, loctag);
    if (type > ASN_CHOICE)
        fprintf(outstr, type_set, name, type);
    if (max)
        fprintf(outstr, sub_boundset, name, min, max);
}

static void print_of(
    int dup,
    long type)
{
    char *c;
    ...
    if ((dup & ASN_DUPED_FLAG))
        fprintf(outstr, dup_func, classname, c, c);
    ...
    char *c)
{
    ...
    if (*c != '.')
        strcpy(hi_end, lo_end); // they are same size
    else
    {
        while (*c == '.')
            c++;
