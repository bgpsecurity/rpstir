    ...
    int num)
{
    ...
    tcasnp = calloc(ncount, sizeof(struct casn));
    ...
    struct casn *casnp)
{
    ...
    if (_clear_error(casnp) < 0)
        return -1;
    if (casnp->type == ASN_CHOICE && !(casnp = _find_filled(casnp)))
        return 0;
    ...
    struct casn *casnp)
{
    ...
    if (!casnp)
        return _casn_obj_err((struct casn *)0, ASN_NULL_PTR);
    return 1;
}
    ...
    long lth)
{
    ...
    if (casnp->type == ASN_UNIVERSAL_STRING)
        return (lth + 3) / 4;
    else if (casnp->type == ASN_BMP_STRING)
        return (lth + 1) / 2;
    ...
    int mode)
{
    ...
    if (!(mode & ASN_READ))
        to = buf;
    c = to;
    if ((lth = _check_filled(casnp, 1)) <= 0)
        return lth;
    if ((casnp->flags & ASN_POINTER_FLAG))
    {
        if (!casnp->ptr)
    ...
    if ((casnp->flags & ASN_DEFAULT_FLAG))
    {
        ...
        if (tcasnp->type == ASN_BOOLEAN)
        {
            if (tcasnp->startp)
                ...
                i = 0;          // does it match the prescribed value?
            ...
            if ((i == 0) == ((tcasnp->min & BOOL_DEFAULT) == 0))
                return 0;
        }
        else
        {
            ...
            if (tcasnp->lth == casnp->lth &&
                !memcmp(tcasnp->startp, casnp->startp, casnp->lth))
                return 0;
        }
    }
    tcasnp = casnp;
    ...
    else if ((i = _readsize(tcasnp, c, mode)) < 0)
        return i - lth;
    c += i;
    lth += i;
    if (contp > to)
        lth += _set_all_lths(to, contp, c, mode);
    return lth;
}
    ...
    int *errp)
{
    if ((casnp->flags & ASN_DEFINED_FLAG))
    {
        if (!(casnp = _find_chosen(casnp)))
            *errp = ASN_NO_DEF_ERR;
    }
    else if (!(casnp = _find_filled(casnp)))
        *errp = ASN_MANDATORY_ERR;
    return casnp;
}
...
**/
    ...
    if ((pflags & ASN_SET_FLAG))
        set_casnp = casnp;      // to go back to each time
    // step 2
    for (num_ofs = did = 0, curr_casnp = casnp; (!nbytes || nbytes > did);)
    {
            ...
            || (pflags & ASN_SET_FLAG))
        {
            ...
            {
                break_out = 0;
                if (curr_casnp->type == ASN_CHOICE &&   // implicit tagged
                                                        // choice
                    // must be explicit
                    curr_casnp->tag == tag &&
                ...
                if ((curr_casnp->flags & ASN_DEFINED_FLAG))
                {
                    ...
                    ch_casnp =
                        _find_tag(&curr_casnp[(pflags & ASN_SET_FLAG) ? 0 : 1],
                                  tag);
                ...
                if (ch_casnp && ch_casnp->type == ASN_ANY)
                    ch_casnp->tag = tag;
                if (!ch_casnp)
        ...
        if (!skip_match)
        {
            ...
            else
            {
                ...
                if (ansr < 0)   // didn't have ASN_NOT_ASN1
                {
                    ...
                    if (curr_casnp->type == ASN_SET)
                        send_flag |= ASN_SET_FLAG;
                    int xlth;
                    if (def_lth)
                        xlth = def_lth;
                    else if (lth == ASN_UNDEFINED_LTH)
                        xlth = lth;
                    else
    ...
    int mode)
{
    ...
    if (casnp->level > 0 && (_go_up(casnp)->flags & ASN_OF_FLAG) &&
        !casnp->ptr)
        return _casn_obj_err(casnp, ASN_OF_BOUNDS_ERR);
    if ((lth = _check_filled(casnp, mode)) <= 0)
        return lth;
    ...
    if ((!(casnp->type & ASN_CONSTRUCTED) && !(casnp->tag & ASN_CONSTRUCTED))
        || !casnp->type || (!(casnp->type & ASN_CONSTRUCTED)
                            && (casnp->flags & ASN_EXPLICIT_FLAG)))
    {
        lth = casnp->lth;
        ...
        if ((mode & ASN_READ))
        {
            ...
            if (tcasnp->type == ASN_BOOLEAN && *c)
                *c = 0xFF;
        }
        if (tcasnp != casnp)
            clear_casn(tcasnp); // free stuff
    }
    else if (casnp->type == ASN_SET && (mode & ASN_READ))
    {
        ...
        tablep = calloc(num + 1, sizeof(struct set_struct));
    ...
    else
    {
        ...
        else
        {
            ...
            do
            {
                if ((i = _encodesize(casnp, c, mode)) < 0)
                    return i - lth;
                lth += i;
                if ((mode & ASN_READ))
                    c += i;     // if chosen, don't go further
                if ((casnp->flags & (ASN_LAST_FLAG | ASN_CHOSEN_FLAG)))
                    break;
                if (ch_casnp)
                    return lth;
                casnp = _skip_casn(casnp, 1);
    ...
    int mode)
{
    ...
    c++;                        // at end of tag-lth;
    if (c < tag_endp)
        lth = _set_all_lths(c, tag_endp, val_endp, mode);
    else
        lth = 0;
    val_endp += lth;
    ...
    int num)
{
    ...
    {
        if ((casnp->flags & ASN_LAST_FLAG))
            return (struct casn *)0;
        for (tcasnp = &casnp[1]; num || (tcasnp->level > casnp->level &&
                                         !(tcasnp->flags & ASN_LAST_FLAG));
             tcasnp++)
        {
            if (tcasnp->level < casnp->level)
                return (struct casn *)0;
            if (tcasnp->level == casnp->level && !(--num))
                break;
        }
        if (!(tcasnp->flags | ASN_LAST_FLAG))
            tcasnp = (struct casn *)0;
    }
    else
    {
        for (tcasnp = &casnp[-1]; num || tcasnp->level > tcasnp->level;
             tcasnp--)
        {
            if (tcasnp->level == casnp->level && !(++num))
                break;
            else if (tcasnp->level < casnp->level)
                return (struct casn *)0;
        }
    }
    return tcasnp;
    ...
    struct casn *casnp)
{
    ...
    {
        for (count = 1, tcasnp = &ucasnp[1]; tcasnp && tcasnp != casnp;
             count++, tcasnp = _skip_casn(tcasnp, 1));
        _stuff_num(count);
    ...
    int lth)
{
    ...
    if (casnp->type == ASN_NONE)
        ...
        err = ASN_CODING_ERR;
    else if (casnp->type == ASN_NULL && lth)
        err = ASN_LENGTH_ERR;
    ...
    else
    {
        ...
        if (tmp >= 0)
            return _casn_obj_err(casnp, ASN_MASK_ERR) - tmp;
    }
    if (err)
        return _casn_obj_err(casnp, err);
    ...
    if (casnp->startp)
        casnp->startp = _free_it(casnp->startp);
    casnp->startp = calloc(1, (casnp->lth = lth));
    ...
    if (*c)
        for (; *c; c++)         /* now do next fields */
        {
            ...
            if (!val)
                ...
                for (tmp = val, siz = 0; tmp; siz++)
                    tmp >>= 7;
            for (a = &buf[i], i += siz, tmp = val, e = b = &a[siz]; siz--;
                 val >>= 7)
                *(--b) =
                    (unsigned char)(val & 0x7F) | ((tmp != val) ? 0x80 : 0);
            if (!*c)
                break;
        }
    casnp->lth = (e - buf);
