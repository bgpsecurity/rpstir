...
**/
    ...
    {
        if ((casnp->flags & ASN_POINTER_FLAG) && casnp->ptr)
            casnp = casnp->ptr;
...
**/
        ...
        {                       /* a tagged CHOICE is implicitly tagged
                                                 ...
                                                 * won't repeat it */
            {
                ...
                if (casnp->type > ASN_CHOICE)
                {
                    ...
                    {
                        i = _dump_tag(casnp->type & ~(ASN_CHOICE), c, offset,
                                      (ushort) 0, mode);
                        if (mode)
                            c += i;
                        ansr += i;
            ...
            if ((casnp->flags & ASN_DEFINED_FLAG)
                ...
                i = (int)_dumpsize(tcasnp, c, offset + 4, mode);
            if (i < 0)
                return i - ansr;
            ansr += i;
            if (mode)
    ...
    if ((tcasnp->type & ASN_CONSTRUCTED))
    {
        ...
        do
        {
                ...
                if (tcasnp->tag == ASN_CHOICE && tcasnp->tag == tcasnp->type)
                {
                        ...
                        !(ttcasnp->flags & ASN_FILLED_FLAG))
                    {
                        if (!(tcasnp->flags & ASN_OPTIONAL_FLAG) &&
                            ...
                            return _casn_obj_err(tcasnp,
                                                 ASN_MANDATORY_ERR) - ansr;
                        continue;
                    }
                }
            ...
            if (of)
                of_casnp = of_casnp->ptr;
        }
        while (of && of_casnp);
        *c = 0;
    }
    else
        ansr += _dumpread(casnp, c, offset + extra, mode);
    return ansr;
}
    ...
    int mode)
{
    ...
    if (type == ASN_IA5_STRING || type == ASN_OCTETSTRING)
    {
        ...
        {
            if (char_table[casnp->startp[i]] & 0x10)
                continue;
            break;
        }
        if (i == lth)
            printable = 1;
             ...
             type == ASN_T61_STRING || type == ASN_UTCTIME
             || type == ASN_GENTIME)
        printable = 1;
    else
        printable = 0;
    ...
    else                        /* output in hex */
    {
        ...
        {
            uchar mask;
            while (lth > 1 && !(casnp->startp[lth - 1]))
                lth--;
            j = 0;
            if (lth > 1)        // have a named bit
            {                   // find how many null bits at least end of
                                // that byte
                for (mask = casnp->startp[lth - 1]; !(mask & 1);
                     j++, mask >>= 1);
            }
        }
        ...
        for (i = 0; i < lth;)
        {
            ...
            for (; j-- && i < lth; i++)
            {
                if ((*c = ((casnp->startp[i]) >> 4) + '0') > '9')
                    *c += 7;
                if (mode)
                    c++;
                if ((*c = ((casnp->startp[i]) & 0xF) + '0') > '9')
                    *c += 7;
                if (mode)
                    c++;
            }
