    ...
    Bag * bag)
{
    ...
    for (it = Bag_const_begin(bag);
         it != Bag_const_end(bag); it = Bag_const_iterator_next(bag, it))
    {
        ++size;
    }
...
#define CONTENTS_TEST(start_iteration, stop_iteration, iterator, begin, end, next, get) \
    ...
    CONTENTS_TEST(Bag_start_const_iteration, Bag_stop_const_iteration,
                  const_it, Bag_const_begin, Bag_const_end,
                  Bag_const_iterator_next, Bag_const_get);
    ...
    Bag * bag)
{
    ...
    if (!empty_test(bag))
        return false;
    if (!correctness_test(bag))
        return false;
    if (!empty_test(bag))
        return false;
    if (!stress_test(bag, 5000))
        return false;
    if (!empty_test(bag))
        return false;
    if (!correctness_test(bag))
...
int main(
    void)
{
    if (!run_test(Bag_new(false)))
        return -1;
    if (!run_test(Bag_new(true)))
        return -1;
