    ...
    char *filename)
{
    ...
    if ((ansr =
        ...
        msg = "getting attribute string";
              ...
              cryptKeysetOpen(&cryptKeyset, CRYPT_UNUSED, CRYPT_KEYSET_FILE,
                              filename, CRYPT_KEYOPT_READONLY)) != 0)
        msg = "opening key set";
    else if ((ansr =
    ...
    else
    {
        ...
        if ((ansr =
             cryptCreateSignature(signature, 200, &signatureLength,
    ...
    char **argv)
{
    ...
    if (setSignature(&roa, tbsp, tbs_lth, argv[2]) < 0)
        fatal(5, (char *)0);
    if (put_casn_file(&roa.self, argv[3], 0) < 0)
        fatal(6, argv[3]);
    return 0;
}
