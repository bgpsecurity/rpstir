    ...
    *(uint32_t *) (buffer + offset - 4) = htonl(pdu->length);
    ...
    if (pdu->pduType == PDU_ERROR_REPORT)
    {
        ...
        *(uint32_t *) (buffer + offset - 4) =
            htonl(pdu->errorData.encapsulatedPDULength);
        ...
        INCR_OFFSET(4);
        *(uint32_t *) (buffer + offset - 4) =
            htonl(pdu->errorData.errorTextLength);

        INCR_OFFSET(pdu->errorData.errorTextLength);
        if (pdu->errorData.errorTextLength > 0)
            memcpy(buffer + offset - pdu->errorData.errorTextLength,
                   pdu->errorData.errorText, pdu->errorData.errorTextLength);
    }
    else
    {
        INCR_OFFSET(pdu->length - PDU_HEADER_LENGTH);
        memcpy(buffer + PDU_HEADER_LENGTH, (void *)pdu + PDU_HEADER_LENGTH,
               pdu->length - PDU_HEADER_LENGTH);

        if (pdu->pduType == PDU_IPV4_PREFIX || pdu->pduType == PDU_IPV6_PREFIX)
        {
            // IP4PrefixData and IP6PrefixData are both in the correct order
            // except for the as_number_t at the end
            *(uint32_t *) (buffer + offset - 4) =
                htonl(*(uint32_t *) (buffer + offset - 4));
        }
                 ...
                 pdu->pduType == PDU_END_OF_DATA)
        {
            ...
            *(uint32_t *) (buffer + PDU_HEADER_LENGTH) =
                htonl(*(uint32_t *) (buffer + PDU_HEADER_LENGTH));
        }
    }
...
static void _fill_pdu_with_serial_number(
    PDU * pdu,
    uint8_t type,
    uint16_t session_or_zero,   // session if the type has one, zero if the
                                // type doesn't
...
void fill_pdu_ipv4_prefix(
    ...
    as_number_t asn)
{
    _fill_pdu_common(pdu, PDU_IPV4_PREFIX,
                     PDU_HEADER_LENGTH + sizeof(pdu->ip4PrefixData));
    pdu->reserved = 0;
...
void fill_pdu_ipv6_prefix(
    ...
    as_number_t asn)
{
    _fill_pdu_common(pdu, PDU_IPV6_PREFIX,
                     PDU_HEADER_LENGTH + sizeof(pdu->ip6PrefixData));
    pdu->reserved = 0;
    ...
    const PDU * pdu)
{
    ...
    if (ret->pduType == PDU_ERROR_REPORT)
    {
        ...
        {
            ret->errorData.encapsulatedPDU =
                malloc(ret->errorData.encapsulatedPDULength);
            if (ret->errorData.encapsulatedPDU == NULL)
            {
                free((void *)ret);
                return NULL;
            ...
            memcpy(ret->errorData.encapsulatedPDU,
                   pdu->errorData.encapsulatedPDU,
                   ret->errorData.encapsulatedPDULength);
        }
        ...
        if (ret->errorData.errorText != NULL)
        {
            ...
            memcpy(ret->errorData.errorText, pdu->errorData.errorText,
                   ret->errorData.errorTextLength);
        }
    }
...
static void _pdu_free_internal(
    PDU * pdu)
{
    if (pdu == NULL)
        return;
...
#define TRUNCATED_STRLEN 4
  ...
  buffer_full:
    if (truncated)
    {
        ...
        {
            strncpy(buffer + PDU_SPRINT_BUFSZ - TRUNCATED_STRLEN - 1,
                    TRUNCATED_STR, TRUNCATED_STRLEN + 1);
        }
    }
