...
3. Running the RPKI Software
    3.1 Normal Operation
      ...
      3.1.2 rpstir-synchronize
      3.1.3 rpstir-rpki-rtr-daemon
      3.1.4 rpstir-rpki-rtr-update
    3.2 Results and Analysis
      3.2.1 Validation Report (rpstir-results)
      3.2.2 Query Client (rpstir-query)
      3.2.3 Log Files
    3.3 Warning
...
you will know:
    ...
    . How to build the rpstir software.
...
Note that rpstir's 64-bit support is in beta. It will probably work as
...
2.1.0 Version Requirements
      OpenSSL (Section 2.1.2) at least 1.0.1g
      MySQL (Section 2.1.3) at least 5.1
      ODBC mySql Connector (Section 2.1.3) at least 3.51
      rsync (Section 2.1.5) at least 2.6.9 
      Python (Section 2.1.6) at least 2.7 

2.1.1 User Account

You must establish a user account from which you can build and run rpstir.
...
specified, the command should be run as rpstir's user.
...
version 1.0.1g) from http://www.openssl.org.  We recommend the default
...
2.13.2).  Then set up a database user and database(s) for rpstir.
...
a root password, then sets up a user with username "rpstir", password "bbn",
and access to the databases named "rpstir" and "rpstir_test":
    ...
    mysql> CREATE USER 'rpstir'@'localhost' IDENTIFIED BY 'bbn';
    mysql> CREATE DATABASE rpstir;
    mysql> GRANT ALL PRIVILEGES ON rpstir.* TO 'rpstir'@'localhost';
    mysql> CREATE DATABASE rpstir_test;
    mysql> GRANT ALL PRIVILEGES ON rpstir_test.* TO 'rpstir'@'localhost';
...
ignored because they will be overridden by rpstir.  However,
...
Install python, at least version 2.7.  Python is likely available in
...
RPSTIR is distributed as source code that should be built
using the GNU build tools on Linux/Unix.

2.2.1 Unpack the Source Code

RPSTIR will have been delivered to you as a gzip-ed tarball
named rpstir-0.11.tar.gz.  Extract the file using:

    $ tar xvzpf rpstir-0.11.tar.gz
    
This will create a directory named rpstir-0.11 which
will contain various subdirectories.

Alternatively, you can get rpstir from git if you want the latest version
...
RPSTIR uses autoconf to generate its Makefiles.  On many systems,
...
Establish a user account from which to run rpstir.  This
...
RPSTIR uses two different configuration files, one for normal
...
Copy etc/test.conf to ~/.rpstir.test.conf and make it readable only by
the owner because it will contain a database password:

   $ cp etc/test.conf ~/.rpstir.test.conf
   $ chmod 600 ~/.rpstir.test.conf

Open ~/.rpstir.test.conf in an editor, read the comments, and edit the
...
to run rpstir-rpki-rtr-daemon and set up an ssh subsystem
...
As root, run "make install" to install rpstir on the system.
...
be used internally or by experts, are in /usr/local/libexec/rpstir.
Documentation, example configuration, and example trust anchor locators are
installed in /usr/local/share/doc/rpstir.  Some internal data is
installed to /usr/local/share/rpstir.  In the next step, you'll manually
create cache and log directories.


2.7 Post-Installation Procedures

If this is the first time you've installed rpstir (i.e. if this is not
an upgrade), there are some initialization steps.

First, create the system-wide configuration directory and file.  The
system-wide configuration directory is /usr/local/etc/rpstir by default.
If you changed the value of ${sysconfdir}, use ${sysconfdir}/rpstir
instead.  Files in the configuration directory should be readable only by the
rpstir user and root.  After creating the directory, create a file
rpstir.conf in it.  There's a sample file that's in
/usr/local/share/doc/rpstir/examples/rpstir.conf by default that you can use as
a starting point.  Assuming everything is in default locations and the user you
created in step 2.1.1 is rpstir with group rpstir:

   # mkdir /usr/local/etc/rpstir
   # cp /usr/local/share/doc/rpstir/examples/rpstir.conf /usr/local/etc/rpstir/
   # chown root:rpstir /usr/local/etc/rpstir /usr/local/etc/rpstir/rpstir.conf
   # chmod 750 /usr/local/etc/rpstir
   # chmod 640 /usr/local/etc/rpstir/rpstir.conf
...
comments, and edit the file as directed by the comments.
   ...
   # vi /usr/local/etc/rpstir/rpstir.conf
...
temporarily by setting $RPSTIR_CONFIG, for example:

   $ RPSTIR_CONFIG=path/to/foo.conf path/to/executable

In the configuration file you just created, find the parameters RPKICacheDir
and LogDir.  Both of these directories, and ${localstatedir}/lib/rpstir,
...
username is rpstir in group rpstir:

   # mkdir -p \
       /usr/local/var/cache/rpstir \
       /usr/local/var/lib/rpstir \
       /usr/local/var/log/rpstir
   # chown rpstir:rpstir \
       /usr/local/var/cache/rpstir \
       /usr/local/var/lib/rpstir \
       /usr/local/var/log/rpstir
...
Then run rpstir-initialize to set up the database:

   $ rpstir-initialize -f # WARNING: This will delete any old data you
...
Consider backing up your configuration and data before upgrading rpstir.

To upgrade rpstir, first upgrade the dependencies to the versions in
section 2.1.0. Then compile (section 2.2) and install (section 2.6) rpstir.
Run rpstir-upgrade with the version number of the previous release. For
...
As part of rpstir's normal operation, it fetches RPKI objects over rsync.
Many of these objects contain links to additional RPKI objects, which rpstir
then attempts to fetch, recursively. If you plan to make the results of rpstir's
...
prevent rpstir from downloading any data it shouldn't.
...
This section describes when and how to run each component of rpstir as
...
If you create cron jobs for rpstir, it is strongly recommended that you
pick random numbers for the fixed parts of the cron job. For example, if you
want to run rpstir once per hour, pick a random number between 0 and 59
to use as the minute that rpstir runs on every hour. Likewise, if you
...
rpstir-synchronize synchronizes the local cache with the global RPKI,
...
rpstir-rpki-rtr-daemon is rpstir's rpki-rtr [rfc6810]
...
After rpstir-synchronize finishes, rpstir-rpki-rtr-update can
update the rpki-rtr cache. If you plan to use the rpki-rtr server, you should
call rpstir-rpki-rtr-update after each successful call to
rpstir-synchronize. For example, the cron job command could be:

   rpstir-synchronize && rpstir-rpki-rtr-update


3.2 Results and Analysis

This section describes some of your options for getting more information about
rpstir's operations (see previous section) or the state of the RPKI.

3.2.1 Validation Report (rpstir-results)

If you want an overview of rpstir's cached view of the RPKI, you can
use

   rpstir-results

This should have some non-zero values for the validated objects if the 
rpstir-synchronize ran successfully.
...
The validation report generator (rpstir-results) searches the local cache
...
rpstir-query -t <type> -d <field> [-d <field> ... -d <field>]
...
is obtained with the command "rpstir-query -l <type>".  Most of these
...
given if you type rpstir-query without any arguments.

    Examples:

    $ rpstir-query -t cert -d pathname      # display paths of valid certs
    $ rpstir-query -t cert -d pathname -d flags # display path+flags of valid certs
    $ rpstir-query -t cert -d pathname -i   # display paths of valid/unknown certs
    $ rpstir-query -t crl -d all            # display all fields of valid CRLs
    $ rpstir-query -t roa -d pathname -d flags # display path+flags of valid roas
    $ rpstir-query -t man -d all -i # display all fields of valid/unknown manifests
    $ rpstir-query -t man -d all -f filename.eq.foo.mft # find all manifests named "foo.mft"

3.2.3 Log Files

RPSTIR logs most things to syslog, so you can check your system logs
...
parameter, which is /usr/local/var/log/rpstir by default.
...
vim ~/.rpstir.test.conf # set these options:
  ...
  DatabaseUser rpstir
  DatabasePassword bbn
make check # There's one test that does not work yet on Mac: tests/conformance/scripts/run_tests.sh
sudo make install
