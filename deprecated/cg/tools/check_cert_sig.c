    ...
    int ffd)
{
    ...
    if (read(ffd, contentsp, name_lth + 2) != name_lth)
        err = ERR_SCM_BADFILE;
              ...
              gen_hash(contentsp, name_lth, contentsp, CRYPT_ALGO_SHA2)) < 0)
        err = ERR_SCM_BADHASH;
    else
    {
    ...
    char *oidp)
{
    ...
    if (num_items(&ch_attrp->attrValues.self) != 1)
        return NULL;
    return ch_attrp;
}
...
#define CHECK(f) if (!(f)) return ERR_SCM_BADSIGINFO
    ...
    if ((usize = vsize_casn(&attrp->attrValues.array.signingTime.utcTime)) >
        15 ||
        (gsize =
         vsize_casn(&attrp->attrValues.array.signingTime.generalizedTime)) >
        17)
        return ERR_SCM_BADSIGINFO;
    if (usize > 0)
    {
...
#undef CHECK
    ...
    if (vsize_casn
        (&rp->content.signedData.signerInfos.signerInfo.sid.
...
{                               // for rsync_aur or anyone else who calls
                                // manifestValidate2
    struct badfile **bpp;
    ...
    struct badfile ***badfilesppp)
{
    ...
    if (read_casn_time(&manp->thisUpdate, &mlo) <= 0 ||
        read_casn_time(&manp->nextUpdate, &mhi) <= 0 || mlo >= mhi)
        return ERR_SCM_BADDATES;
         ...
         fahp; fahp = (struct FileAndHash *)next_of(&fahp->self))
    {
        ...
        if (dir_lth)
            strcat(strncpy(path, dirp, dir_lth), "/");
        strcat(path, fname);
        tmp = 0;
        if ((ffd = open(path, O_RDONLY)) < 0)
            tmp = ERR_SCM_COFILE;
        else
            tmp = check_fileAndHash(fahp, ffd);
        if (tmp < 0)            // add the file to the list
        {
            if (numbadfiles == 0)
                badfilespp =
                    (struct badfile **)calloc(2, sizeof(struct badfile *));
            else
                badfilespp =
                    (struct badfile **)realloc(badfilespp,
                                               ((numbadfiles +
                                                 2) *
                                                sizeof(struct badfile *)));
            struct badfile *badfilep =
                (struct badfile *)calloc(1, sizeof(struct badfile));
    ...
    struct ROA *manp)
{
    ...
    struct casn *casnp =
        &manp->content.signedData.encapContentInfo.eContent.manifest.version.
        self;
    long val;
    if (size_casn(casnp) > 0 && (read_casn_num(casnp, &val) > 1 || val > 0))
    ...
    struct ROA *rp)
{
    ...
    // check that eContentType is routeOriginAttestation (= OID
    // 1.2.240.113549.1.9.16.1.24)
    if (diff_objid(&rp->content.signedData.encapContentInfo.eContentType,
    ...
    // check that the version is right
    struct casn *casnp =
        &rp->content.signedData.encapContentInfo.eContent.roa.version.self;
    long val;
    if (size_casn(casnp) > 0 && read_casn_num(casnp, &val) > 1 && val > 0)
        return ERR_SCM_INVALVER;
    // check that the asID is a positive nonzero integer
    if (read_casn_num
        (&rp->content.signedData.signerInfos.signerInfo.version.self,
         &iAS_ID) < 0 || iAS_ID <= 0)
        return ERR_SCM_INVALASID;
    // check the contents
    if ((iRes =
         validateIPContents(&rp->content.signedData.encapContentInfo.eContent.
                            roa.ipAddrBlocks)) < 0)
        return iRes;
    return 0;
}
