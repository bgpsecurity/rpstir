...
# refresh test certs
  ...
  run "update_cert" $RPKI_ROOT/cg/tools/update_cert 0D 2Y C?*.cer
  cd ../
...
# clear database
    ...
    run_bg "rcli-w-$N" ../proto/rcli -w $RPKI_PORT
    LOADER_PID=$!
     ...
     run "rcli-c-$N" ../proto/rcli -c testcases4_LTA/LTA/case${N} || failures="$failures rcli-c"
     cd testcases4_LTA/LTA 
     run "checkLTAtest-$N" $RPKI_ROOT/cg/tools/checkLTAtest case${N} C*.cer || failures="$failures checkLTAtest"
     cd ../../
    if [ -z "$failures" ]; then
