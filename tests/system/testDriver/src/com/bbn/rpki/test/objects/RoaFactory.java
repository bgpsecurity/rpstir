...
public class RoaFactory extends Factory<AllocateROAAction> implements Constants {
	 ...
	 * @param a
	 */
	public RoaFactory(String name, List<Pair> ipv4, List<Pair> ipv6,
			List<Pair> as_list, List<Pair> child, String server,
			List<Pair> roav4l, List<Pair> roav6l, int a) {
	...
	/**
	 * @see com.bbn.rpki.test.objects.FactoryBase#create(com.bbn.rpki.test.tasks.Model,
	 *      com.bbn.rpki.test.objects.CA_Object, int)
	 */
	@Override
	public AllocateROAAction create(Model model,
			InitializeAction initializeAction, CA_Object parent, int id) {
		if (DEBUG_ON) {
		...
		if (ipCount > 0) {
			...
			return new AllocateROAAction(parent, AllocationId.get("roa-ini-"
					+ parent.getNickname()), model, allPairs);
		}
		return null;
	}
