 ...
 * Uploads a model.
 * 
 * There are two breakdowns: byRepositoryRoot -- provides tasks for uploading
 * individual roots byNode -- provides tasks for uploading individual nodes
 * 
 * @author tomlinso
 */
public class UploadEpoch extends TaskFactory {
	...
	protected class Task extends TaskFactory.Task {
		...
		private List<TaskFactory.Task> getSubtasks() {
			...
			List<TaskFactory.Task> ret = new ArrayList<TaskFactory.Task>(
					roots.size());
			UploadRepositoryRoot factory = model
					.getTaskFactory(UploadRepositoryRoot.class);
			for (File repositoryRootDir : roots) {
				String repositoryRootName = model
						.getRepositoryRootName(repositoryRootDir);
		...
		protected String getLogDetail() {
			if (getSelectedTaskBreakdown() != null) {
				return getSelectedTaskBreakdown().getTasks().size()
						+ " sub-tasks";
			}
