...
public class CAFactory extends Factory<CA_Object> {
	 ...
	 * @param subjKeyFile
	 */
	protected CAFactory(String bluePrintName, List<Pair> ipv4List,
			List<Pair> ipv6List, List<Pair> asList, List<Pair> childSpec,
			String serverName, String subjKeyFile) {
		super(bluePrintName, ipv4List, ipv6List, asList, childSpec, serverName,
				subjKeyFile);
	}
	 ...
	 */
	@Override
	CA_Object create(Model model, InitializeAction initializeAction,
			CA_Object parent, int id) {
		CA_Object caObject = new CA_Object(parent, bluePrintName + "-" + id,
				serverName, subjKeyFile);
		TypedPair[] allPairs = new TypedPair[asList.size() + ipv4List.size()
				+ ipv6List.size()];
		if (allPairs.length > 0) {
			...
			AbstractAction allocateAction = new AllocateAction(parent,
					caObject,
					AllocationId.get("ini-" + caObject.getNickname()), model,
					allPairs);
	...
	private int addPairs(TypedPair[] allPairs, int q, IPRangeType rangeType,
			List<Pair> pairs) {
		for (Pair pair : pairs) {
