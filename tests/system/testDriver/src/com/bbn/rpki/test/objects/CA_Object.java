 ...
 * may be a current certificate reflecting allocations issued (and published) by
 * another CA. The validity period of the certificate is determined from the set
 * of active allocations received by this CA. The validity start time is the
 * latest start time of any active allocation and the validity end time is the
...
public class CA_Object extends Allocator {
	 ...
	 * @param subjKeyFile
	 */
	public CA_Object(CA_Object parent, String nickname, String serverName,
			String subjKeyFile) {
		this.nextChildSN = 0;
	...
	public CA_Object(CA_Object parent, Element element) {
		this(parent, element.getAttributeValue(XMLConstants.ATTR_NICKNAME),
		...
		if (isModified()) {
			if (this.certificate != null) {
				if (!this.certificate.hasExpired()) {
					parent.revokedCertificates.add(new RevokedCertificate(
							this.certificate));
				}
			...
			if (hasResources()) {
				...
				if (parent != null) {
					...
					this.certificate = new CA_cert(parent, validityStartTime,
							validityEndTime, dirPath, nickName, getSIA_path(),
							getRcvdRanges(IPRangeType.as),
							getRcvdRanges(IPRangeType.ipv4),
							getRcvdRanges(IPRangeType.ipv6), this.subjKeyFile);
				} else {
					String dirPath = REPO_PATH + getServerName() + "/";
					this.certificate = new SS_cert(parent, validityStartTime,
							validityEndTime, getSIA_path(), nickName, dirPath,
							getRcvdRanges(IPRangeType.as),
							getRcvdRanges(IPRangeType.ipv4),
							getRcvdRanges(IPRangeType.ipv6), subjKeyFile);
				}
	 ...
	 * @param allocationId
	 */
	public void takeAllocation(List<? extends Pair> pairs,
			IPRangeType rangeType, long validityStartTime,
			long validityEndTime, AllocationId allocationId) {
		IPRangeList allocation = parent.subAllocate(rangeType, pairs);
		addRcvdRanges(validityStartTime, validityEndTime, allocationId,
				allocation);
	}
	 ...
	 * @param allocations
	 */
	public void addRcvdRanges(long validityStartTime, long validityEndTime,
			AllocationId allocationId, IPRangeList... allocations) {
		ActionManager.singleton().recordAllocation(parent, this, allocationId,
				allocations);
	...
	public void returnAllocation(AllocationId allocationId) {
		for (IPRangeType rangeType : IPRangeType.values()) {
			IPRangeList allocation = ActionManager.singleton().findAllocation(
					parent, this, rangeType, allocationId);
			if (allocation != null) {
	...
	public void appendObjectsToWrite(List<CA_Obj> list) {
		...
		if (cert != null) {
			...
			// Do this after processing children because a certificate may be
			// revoked
			{
	...
	/**
	 * @param path
	 *            navigation down to requested descendant
	...
	/**
	 * @param action
	 *            the action to perform on every CA_Object
	 */
	...
	public List<RevokedCert> getRevokedCertList() {
		List<RevokedCert> ret = new ArrayList<RevokedCert>(
				revokedCertificates.size());
		for (Iterator<RevokedCertificate> it = revokedCertificates.iterator(); it
				.hasNext();) {
			...
			if (cert.hasExpired()) {
				...
				ret.add(new RevokedCert(cert.serial, new Date(
	...
	public String getSIA_path() {
		if (parent != null) {
			return isBreakAway() ? getServerName() + "/" + nickName + "/"
					: parent.getSIA_path() + nickName + "/";
		} else {
