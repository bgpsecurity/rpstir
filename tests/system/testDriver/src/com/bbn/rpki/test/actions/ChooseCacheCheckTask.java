 ...
 * that task as being a task after which cache testing should be performed. The
 * path is specified as an alternation of task names and breakdown names. The
 * first element selects a top task, the second element selects a particular
...
public class ChooseCacheCheckTask extends AbstractAction {
   ...
   */
	...
	enum AttributeType {
		TASK_PATH("Task Path"), ID("Id"), EPOCH("Epoch");
	 ...
	 *      com.bbn.rpki.test.objects.TypescriptLogger)
	 */
	@Override
	...
	/**
	 * This action causes the upload task to be broken down into individual
	 * upload tasks. This effects the upload time, but not the computation time
	 * 
	...
	public boolean referencesCA(CA_Object caObject) {
		...
		// We should say true if the test task is one that references only the
		// specified CA
		return false;
	}
