...
public class IPRangeList implements Iterable<Range>, Constants {
	...
	public static final IPRangeList IPV4_INHERIT = new IPRangeList(
			IPRangeType.ipv4);

	/** Indicates the address range is inherited */
	public static final IPRangeList IPV6_INHERIT = new IPRangeList(
			IPRangeType.ipv6);
	...
	public static final IPRangeList IPV4_EMPTY = new IPRangeList(
			IPRangeType.ipv4);

	/** Predefined constants for an empty range */
	public static final IPRangeList IPV6_EMPTY = new IPRangeList(
			IPRangeType.ipv6);
	...
	public static boolean isInherit(IPRangeList l) {
		...
		case ipv4:
			return l == IPV4_INHERIT;
		case ipv6:
			return l == IPV6_INHERIT;
		case as:
			return l == AS_INHERIT;
		}
		return false;
	...
	public static void main(String... args) {
		...
		Range range1 = new Range(new BigInteger("1000"),
				new BigInteger("1999"), IPRangeType.as, true);
		Range range2 = new Range(new BigInteger("2000"),
				new BigInteger("2999"), IPRangeType.as, true);
		rcvd.add(range1);
		rcvd.add(range2);
		IPRangeList free = new IPRangeList(rcvd);
		// Allocate a prefix that must start at 1024 cutting a hole
		IPRangeList suballoc = free.allocate(
				Collections.singletonList(new Pair("p", 1024)), true);
	 ...
	 * "amount" field must be a positive integer specifying the number of
	 ...
	 * @return
	 */
	private Range allocateSinglePrefix(BigInteger reqSize,
			IPRangeList allocatedList, boolean expressAsRange) {
		if (!Range.isPowerOfTwo(reqSize)) {
			throw new AllocationError("Illegal prefix size: " + reqSize);
		}
		for (int i = 0, n = rangeList.size(); i < n; i++) {
			Range x = firstFitPrefix(rangeList.get(i), allocatedList, reqSize,
					expressAsRange);
			if (x != null) {
				removeRange(i, x);
				return x;
		...
		throw new AllocationError(
				"Unable to fulfill request for prefix of size " + reqSize);
	}
	...
	private Range allocateSingleRange(BigInteger reqSize,
		...
		for (int i = 0, n = rangeList.size(); i < n; i++) {
			...
			Range x = firstFitRange(firstRange, reqSize, allocatedBlocks,
					expressAsRange);
			if (x != null) {
		...
		throw new AllocationError(
				"Unable to fulfill request for range of size " + reqSize);
	}
	 ...
	 * @return
	 */
	private Range firstFitPrefix(Range free_block,
			IPRangeList allocated_blocks, BigInteger amount,
			boolean expressAsRange) {
		BigInteger search_position = free_block.min;
		while (true) {
			Range candidate = next_prefix(search_position, amount,
					allocated_blocks.getIpVersion(), expressAsRange);
	...
	private Range next_prefix(BigInteger start_pos, BigInteger amount,
			IPRangeType version, boolean expressAsRange) {
		// Return the next prefix of the requested size.

		if (!Range.isPowerOfTwo(amount)) {
			throw new AllocationError(String.format(
					"Prefix request has invalid size: %d.", amount));
		}
		BigInteger next_multiple;
		if (start_pos.mod(amount).equals(BigInteger.ZERO)) {
			next_multiple = start_pos;
		} else {
			next_multiple = start_pos.divide(amount).add(BigInteger.ONE)
					.multiply(amount);
		}
		Range prefix = new Range(next_multiple, next_multiple.add(amount)
				.subtract(BigInteger.ONE), version, expressAsRange);
	 ...
	 * @return
	 */
	private Range firstFitRange(Range freeRange, BigInteger reqSize,
			IPRangeList allocatedBlocks, boolean expressAsRange) {
		BigInteger searchPosition = freeRange.min;
		while (true) {
			Range candidate = new Range(searchPosition, searchPosition.add(
					reqSize).subtract(BigInteger.ONE), freeRange.version,
					expressAsRange);
	 ...
	 * @return
	 */
	private static Range detectConflict(Range candidate,
		...
		Range expanded = new Range(candidate.min.subtract(BigInteger.ONE),
				candidate.max.add(BigInteger.ONE), candidate.version, true);
		...
		for (int i = 0, n = rangeList.size(); i <= n; i++) {
			...
			if (test == null || test.compareTo(range) > 0) {
				...
				case 1:
					...
					rangeList.set(i, new Range(range.min, test.max,
							test.version, false));
					return;
				case 2:
					// Adjacent to preceding range
					rangeList.set(i - 1, new Range(test2.min, range.max,
							test2.version, false));
					return;
				case 3:
					// adjacent to both
					rangeList.set(i - 1, new Range(test2.min, test.max,
							test.version, false));
					rangeList.remove(i);
					return;
				}
	...
	public boolean remove(Range range) {
		...
		for (int i = 0, n = rangeList.size(); i < n; i++) {
			...
			if (!test.contains(range)) {
				// Error -- trying to remove a range that is not completely
				// present.
				return false;
			}
