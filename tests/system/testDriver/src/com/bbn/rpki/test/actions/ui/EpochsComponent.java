...
class EpochsComponent extends JPanel {
	...
	EpochsComponent(Model model, final EpochEvent epoch, final String title,
			final Collection<EpochEvent> currentSelection,
			Collection<EpochEvent> availableEpochs,
			final ActionDetail.EpochCallback cb) {
		...
		removeButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				for (EpochEvent epochEventToRemove : list
		...
		addButton
				.setToolTipText(availableEpochs.isEmpty() ? "No epochs available"
						: "Add epoch from list");
	...
	public void enableRemove(boolean b) {
		...
		removeButton.setToolTipText(b ? "Remove the selected epoch"
				: "No epoch selected");
	}
	...
	void addEpoch() {
		...
		int option = JOptionPane.showConfirmDialog(this, msg, "Select Epoch",
