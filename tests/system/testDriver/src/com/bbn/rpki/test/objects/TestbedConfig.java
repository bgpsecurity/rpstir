...
public class TestbedConfig implements Constants {
	...
	/**
	 * Parses the val as a string like AFRINIC,1%APNIC,2%RIPE,2... Stores result
	 * as a tuple in the list toMode
	 */
	...
	public TestbedConfig(String iniFile) {
		...
		if (!objectsDir.mkdir() && objectsDir.exists()) {
		...
		try {
			...
			for (Map.Entry<String, Section> sectionEntry : sectionEntries) {
				...
				for (TestbedCreate.Option option : TestbedCreate.Option
						.values()) {
					if (!option.keep()) {
	...
	private void configureFactory(Map.Entry<String, Section> sectionEntry) {
		...
		for (Map.Entry<String, String> entry : sectionMap.entrySet()) {
			TestbedCreate.Option option = TestbedCreate.Option.valueOf(entry
					.getKey());
			if (option == null) {
				System.err.println("Opt in config file not recognized: "
						+ entry.getKey());
				continue;
			}
		...
		TestbedCreate.FactoryType factoryType = TestbedCreate.FactoryType
				.valueOf(type);
		if (factoryType == null) {
