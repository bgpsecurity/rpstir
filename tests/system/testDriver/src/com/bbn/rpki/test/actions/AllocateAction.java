 ...
 * don't. For testing, however, it is necessary to cover the cases where an
 * allocation is rescinded and the associated timing of when the world becomes
 * aware of this.
 * 
 * @author tomlinso
 */
public class AllocateAction extends AllocateActionBase {
	 ...
	 */
	public AllocateAction(CA_Object parent, CA_Object child,
			AllocationId allocationId, Model model, TypedPair... pairs) {
	...
	public AllocateAction(Model model) {
		this(model.getRootCA(), getFirstChild(model), AllocationId.generate(),
				model);
	}
	 ...
	 * @param actionContext
	 */
	public AllocateAction(Element element, Model model,
	 ...
	 * 
	 * @see com.bbn.rpki.test.actions.AbstractAction#execute(EpochEvent,
	 *      TypescriptLogger)
	 */
	@Override
	public void execute(EpochEvent epochEvent, TypescriptLogger logger) {
		...
		case PUBLICATION_TIME_OF_ALLOCATION:
			if (child == parent || parent == null) {
				...
				child.addRcvdRanges(
						...
						allocationId, getEverything(IPRangeType.as),
				...
				for (TypedPair typedPair : allocationPairs) {
					...
					case ipv4:
						child.takeAllocation(Collections
								.singletonList(typedPair), IPRangeType.ipv4,
								validityStartTime.getEpoch().getEpochTime(),
								validityEndTime.getEpoch().getEpochTime(),
								allocationId);
						break;
					case ipv6:
						child.takeAllocation(Collections
								.singletonList(typedPair), IPRangeType.ipv6,
								validityStartTime.getEpoch().getEpochTime(),
								validityEndTime.getEpoch().getEpochTime(),
								allocationId);
						break;
					case as:
						child.takeAllocation(Collections
								.singletonList(typedPair), IPRangeType.as,
								validityStartTime.getEpoch().getEpochTime(),
								validityEndTime.getEpoch().getEpochTime(),
								allocationId);
						break;
				...
				if (logger != null) {
					logger.format(
							"Allocate %s from %s to %s identified as %s%n",
							allocationPairs, parent, child, allocationId);
				}
			}
			break;
		case VALIDITY_END_TIME_OF_ALLOCATION:
			if (child != parent) {
				...
				if (logger != null) {
					logger.format(
							"Deallocate %s from %s to %s identified as %s%n",
							allocationPairs, parent, child, allocationId);
				}
			}
			break;
	...
	private IPRangeList getEverything(IPRangeType rangeType) {
		...
		everything.add(new Range(BigInteger.ZERO, rangeType.getMax(),
				rangeType, rangeType == IPRangeType.as));
		return everything;
	}

	/**
	 * @see com.bbn.rpki.test.actions.AbstractAction#addExecutionTime(com.bbn.rpki.test.actions.EpochEvent,
	 *      com.bbn.rpki.test.actions.ExecutionTimeContext)
	 */
	@Override
	public void addExecutionTime(EpochEvent epochEvent,
			ExecutionTimeContext etContext) {
		switch (epochEvent.getName()) {
		case PUBLICATION_TIME_OF_ALLOCATION:
	...
	public String toString() {
		String parentNickname = parent == null ? "<unspecified>" : parent
				.getNickname();
		String childNickname = child == null ? "<unspecified>" : child
				.getNickname();
		return String.format("Allocate %s: %s from %s to %s", allocationId,
				allocationPairs, parentNickname, childNickname);
	}
	...
	public List<String> getInvalidReasons() {
		...
		if (childParent != null && childParent != child) {
			if (childParent != getParent()) {
				ret = appendReason(ret,
						"Subject CA must be a child of the issuer CA");
			}
		}
		return ret;
	}
