...
public class TestbedCreate implements Constants {
	enum Option {
		childSpec, ipv4List(false), ipv6List(false), asList(false), serverName, breakAway, ttl, max_depth, max_nodes, ROAipv4List(
				false), ROAipv6List(false), asid(false), subjkeyfile;
   ...
   */
	...
	public InitializeAction create_driver(Model model, CA_Object iana) {
		...
		while (!(ca_queue.isEmpty() && child_queue.isEmpty())
				&& MAX_DEPTH > repo_depth && MAX_NODES > repo_size) {
		...
		System.out
				.format("Finished creation driver loop. repo_depth = %d repo_size = %d%n",
						repo_depth, repo_size);
		...
		for (Pair ca_def : list) {
			for (int n = 0; n < ca_def.arg.intValue(); n++) {
				if (MAX_NODES > repo_size) {
					...
					Object child = childFactory.create(model, initializeAction,
							ca_node, ca_node.getChildCount());
					if (child instanceof CA_Object) {
						...
						System.err
								.println("Somehow got something besides CA or ROA as a child");
					}
					repo_size += 1;
					if (DEBUG_ON) {
						System.out.format("Child created. repo_size = %d%n",
								repo_size);
					}
				} else {
					return repo_size;
