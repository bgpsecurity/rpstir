...
public class Model implements Constants, XMLConstants {
	...
	static class TaskFactoryKey {
		 ...
		 * @param arg
		 */
		protected TaskFactoryKey(Class<? extends TaskFactory> factoryClass,
				Object arg) {
			super();
	 ...
	 * @throws IOException
	 */
	public Model(File rpkiRoot, Element rootElement, TypescriptLogger logger)
			throws IOException {
	...
	public void addTask(TaskFactory.Task task) {
		...
		if (shouldUpdateCache(task)) {
			...
			addTaskInner(getTaskFactory(CheckCacheStatus.class)
					.createOnlyTask());
		}
	}
	 ...
	 * of an epoch is recorded in a data object, that start time and all
	 * preceding start times become fixed and are not allowed to be updated.
	 ...
	 * epoch. 4) Determine which of the objects are newly written, untouched, or
	 * superceded (deleted). 5) Finally, determine the new set of roots.
	 */
	public void advanceEpoch() {
		...
		if (delay <= 0) {
			System.err.format("Epoch %d late by %d milliseconds%n", epochIndex,
					-delay);
		} else {
			try {
				System.out.format("Epoch %d delay by %d milliseconds%n",
						epochIndex, delay);
				Thread.sleep(delay);
		...
		for (File serverFile : REPO_DIR.listFiles(dirFilter)) {
			...
			repositoryRoots.addAll(Arrays.asList(serverFile
					.listFiles(dirFilter)));
		}
		++epochIndex;
	}
	 ...
	 * @return get the path of a publicationSource relative to the source
	 *         repository directory broken into parts
	 */
	...
	/**
	 * @return the root CA TODO local trust anchors mean multiple roots.
	 */
	...
	public TaskFactory.Task[] getTopTasks() {
		...
		TaskFactory.Task[] tasks = { factory.createOnlyTask() };
		return tasks;
	}
	...
	/**
	 * Find the Epochs that can be successors of the given Epoch. This set is
	...
	/**
	 * Find the Epochs that can be predecessors of the given Epoch. This set is
	...
	/**
	 * Find the Epochs that can be coincident with the given Epoch. This set is
