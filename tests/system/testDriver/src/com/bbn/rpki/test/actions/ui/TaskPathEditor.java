...
public class TaskPathEditor {
	private static class Item {
		...
		/**
		 * @return true if this item is valid Only the last item can be invalid.
		 */
	...
	public void setTaskPath(TaskPath taskPath) {
		...
		if (path.length == 0) {
			...
			addBase(null, 0);
			path = new String[] { items.get(0).task.getTaskName() };
		} else {
			addBase(path[0], 0);
		}
	...
	private void addBase(String name0, int index) {
		...
		if (name0 != null) {
			...
			try {
				taskClass = (Class<? extends TaskFactory>) Class
						.forName(rootClassName);
	...
	private void addBreakdown(String breakdownName, final int index) {
		...
		if (breakdownName == null) {
			...
			final Breakdown currentBreakdown = factory
					.getBreakdown(breakdownName);
	...
	private void addTaskFactory(String taskName, final int index) {
		...
		final Item item = new Item();
		TaskBreakdown taskBreakdown = breakdown
				.getTaskBreakdown(previousTaskItem.task);
