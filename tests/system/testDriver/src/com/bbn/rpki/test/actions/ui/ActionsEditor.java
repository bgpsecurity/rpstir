...
public class ActionsEditor implements SelectionListener {
	...
	private final Action addAction = new javax.swing.AbstractAction(
			"Add Action") {
		@Override
		public void actionPerformed(ActionEvent e) {
			Class<?>[] classes = { AllocateAction.class,
					AllocateROAAction.class, ChooseCacheCheckTask.class };
			String[] names = new String[classes.length];
			for (int i = 0; i < names.length; i++) {
				names[i] = classes[i].getSimpleName();
			}
			JComboBox box = new JComboBox(names);
			int option = JOptionPane.showConfirmDialog(treePane, box,
					"Select a Type of Action", JOptionPane.OK_CANCEL_OPTION,
			...
			if (option == JOptionPane.OK_OPTION) {
				...
				if (index >= 0) {
					...
					try {
						Constructor<?> constructor = chosenClass
								.getConstructor(Model.class);
						AbstractAction addedAction = (AbstractAction) constructor
								.newInstance(model);
	...
	private final Action deleteAction = new javax.swing.AbstractAction("Delete") {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (pathCount == 3) {
				EpochEvent epoch = (EpochEvent) selectedPath
						.getPathComponent(2);
	...
	private final Action editCAListAction = new javax.swing.AbstractAction(
			"Edit CA List") {

		@Override
		public void actionPerformed(ActionEvent e) {
			CAEditor caEditor = new CAEditor(model, model.getRootCA(),
					getComponent());
			caEditor.showDialog();
		}
	...
	private final Action collapseAction = new javax.swing.AbstractAction(
			"Collapse") {
		@Override
		public void actionPerformed(ActionEvent e) {
			actionTree.collapse();
	...
	public ActionsEditor(Model model) {
		...
		JButton[] buttons = { new JButton(addAction),
				new JButton(deleteAction), new JButton(editCAListAction),
				new JButton(expandAction), new JButton(collapseAction) };
