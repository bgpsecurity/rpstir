 ...
 * 
 * Records the numbers of object updates of various kinds that will occur and
 * the task breakdowns that will upload the objects. Also recorded are cache
 * check invocations.
 * 
 * For example, there is execution time overhead associated with performing an
 * scp to a repository publication point. This overhead is incurred only once
 * per publication point per upload task. Additionally there is time spent per
 * file and per byte. This version applies the per file burden, but ignores the
 * per byte burden.
 * 
 * @author rtomlinson
 */
