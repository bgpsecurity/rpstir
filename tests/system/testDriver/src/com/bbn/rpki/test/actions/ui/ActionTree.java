...
public class ActionTree {
	...
	public ActionTree(Model model, final SelectionListener l) {
		...
		actionTree.getSelectionModel().setSelectionMode(
		...
		model.addListener(new Model.Listener() {
			...
			public void epochsChanged() {
				// Save the current selection and try to re-establish after
				// changing the
				// model
				Enumeration<TreePath> expandedDescendents = actionTree
						.getExpandedDescendants(new TreePath(actionTreeModel
								.getRoot()));
				...
				if (expandedDescendents != null) {
					while (expandedDescendents.hasMoreElements()) {
						...
						if (last instanceof Epoch) {
							visibleEpochEvents.addAll(((Epoch) last)
									.getEpochEvents());
						}
					}
				}
				...
				for (EpochEvent epochEvent : visibleEpochEvents) {
					TreePath path = actionTreeModel.findPathTo(epochEvent
							.getEpoch());
				...
				if (selectionPath != null) {
					...
					if (lastComponent instanceof EpochEvent) {
						// Try to re-select the same epoch after shuffling
						// epochs
						TreePath newPath = actionTreeModel
								.findPathTo(lastComponent);
