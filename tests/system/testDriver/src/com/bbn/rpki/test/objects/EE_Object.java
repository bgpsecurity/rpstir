...
public class EE_Object extends Allocator {
	...
	public EE_Object(long validityStartTime, long validityEndTime,
			List<Pair> asList, List<Pair> ipv4List, List<Pair> ipv6List,
			String bluePrintName, CA_Object parent) {
	...
	public EE_cert getCertificate() {
		if (this.certificate == null || isModified()) {
			this.certificate = new EE_cert(parent, validityStartTime,
					validityEndTime, bluePrintName + "-" + id,
