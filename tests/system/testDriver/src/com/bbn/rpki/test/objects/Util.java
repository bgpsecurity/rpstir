...
public class Util implements Constants {
	...
	private static final String B64_ALPHABET = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_";

	private static DateFormat dateFormat1 = new SimpleDateFormat(
			"yyMMddHHmmss'Z'");
	...
	private static StringBuilder appendList(StringBuilder sb, String member,
			Iterable<?> words, boolean bracketed) {
	...
	public static void writeConfig(CA_Obj ca_obj) {
		try {
			...
			// loops through all member of this class and writes them to the
			// config
			// file
			...
			for (Map.Entry<String, Object> entry : fieldMap.entrySet()) {
				...
				if (val != null) {
					...
					if (member.equals("issuer") || member.equals("subject")) {
						...
						String ser = fieldNames.length < 2 ? null
								: fieldNames[1];
						if (ser != null) {
							sb.append(String.format("%s=%s%%%s%n", member,
									name2, ser));
						} else {
						...
						if (IPRangeList.isInherit(rangeList)) {
							sb.append(String.format("%s=%s%n", member,
									"inherit"));
						} else {
					...
					} else if (member.equals("roaipv4")
							|| member.equals("roaipv6")) {
						IPRangeList rangeList = (IPRangeList) val;
						appendList(sb, member, rangeList, false);
					} else if (member.equals("notBefore")
							|| member.equals("notAfter")) {
						appendDateTime(sb, member, (Calendar) val, true);
					} else if (member.equals("thisupdate")
							|| member.equals("nextupdate")) {
						Calendar cal = (Calendar) val;
						appendDateTime(sb, member, cal,
								!(ca_obj instanceof Manifest));
	 ...
	 * @param cal
	 */
	private synchronized static void appendDateTime(StringBuilder sb,
			String member, Calendar cal, boolean abbrevYear) {
	 ...
	 */
	public static String exec(String title, boolean ignoreStatus, File cwd,
			String input, String cleanCommand, List<String> cmds) {
		return exec(title, ignoreStatus, false, cwd, input, cleanCommand,
	 ...
	 * @return stdout string
	 */
	public static String exec(String title, boolean ignoreStatus,
			boolean showStdOut, File cwd, String input, String cleanCommand,
			String... cmdArray) {
		int status;
		try {
			if (cwd == null) {
			...
			if (showStdOut && typescriptLogger != null) {
				stdoutReader = typescriptLogger.addSource(stdoutReader,
						"stdout");
				stderrReader = typescriptLogger.addSource(stderrReader,
						"stderr");
			}
			...
			commandLog.flush();
			if (status != 0) {
				String msg = String.format("%s %s status = %d%n", title,
						ignoreStatus ? "ignored" : "failed", status);
				if (DEBUG_ON && typescriptLogger != null) {
					typescriptLogger.log(ignoreStatus ? "stdout" : "stderr",
							msg);
					if (!ignoreStatus) {
	...
	public static String removePrefix(String string, String prefix) {
		// TODO figure out why string sometimes starts with s: whereas prefix
		// starts
		// with r:
	...
	public static void killProcessesRunning(String string) {
		...
		String psOutput = Util.exec("ps -aef", true, false, null, null, null,
				"ps", "-aef");
