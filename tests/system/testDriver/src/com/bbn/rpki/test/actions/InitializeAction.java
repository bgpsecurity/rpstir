...
public class InitializeAction {
	 ...
	 */
	public List<AbstractAction> getActions(Model model, CA_Object iana,
			List<AbstractAction> constraintActions) {
		constrainValidityStartTimes(model, iana, constraintActions);
		return actions;
	}

	private void constrainValidityStartTimes(Model model, CA_Object iana,
			List<AbstractAction> constraintActions) {
		...
		if (ianaAllocateAction == null) {
			ianaAllocateAction = new AllocateAction(iana, iana,
					AllocationId.get("ini-iana"), model);
		...
		for (AbstractAction action : actions) {
			...
			if (action instanceof AllocateActionBase) {
				...
				rcvdAction.getValidityStartEvent().addSuccessor(
						allocateAction.getValidityStartEvent(), true);
				rcvdAction.getValidityEndEvent().addPredecessor(
						allocateAction.getValidityEndEvent(), true);
				for (AbstractAction constraintAction : constraintActions) {
					allocateAction.constrainBy(constraintAction
							.getAllEpochEvents());
				}
			}
		}
