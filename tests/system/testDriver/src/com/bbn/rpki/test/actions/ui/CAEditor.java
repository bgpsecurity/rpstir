...
/**
 * Provides a dialog for selecting a CA. The dialog displays the CA hierarchy as
 * a tree
 * 
 * @author tomlinso
 */
public class CAEditor implements CAChooser.Listener {
	...
	private final Action deleteAction = new javax.swing.AbstractAction(
			"Delete CA") {
   ...
   */
	protected void deleteCA() {
		...
		Object[] msg = {
				selectedCA.getCommonName()
						+ " has "
						+ (descendentCount == 0 ? "no" : String
								.valueOf(descendentCount)) + " descendent"
						+ (descendentCount == 1 ? "" : "s"),
				"and",
				selectedCA.getCommonName()
						+ " and descendents are referenced by "
						+ (actionCount == 0 ? "no" : String
								.valueOf(actionCount)) + " action"
						+ (actionCount == 1 ? "" : "s"),
				"Are you sure you want to delete it and all descendents?" };
		int option = JOptionPane.showConfirmDialog(dialog, msg,
				"Confirm CA Deletion", JOptionPane.OK_CANCEL_OPTION);
	...
	private void addDescendentActions(CA_Object caObject,
   ...
   */
	...
	private void updateButtonEnables() {
		...
		deleteAction.setEnabled(selectedCA != null
				&& selectedCA.getParent() != null);
	}
