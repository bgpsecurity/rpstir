 ...
 * 
 * The specific files to be deleted are obtained from the model as determined by
 * the changes that have occured in the model
 * 
 * @author tomlinso
 */
	...
	protected void appendBreakdowns(List<Breakdown> list) {
		for (String bdn : xHandler.getBreakdownNames()) {
			...
			list.add(new Breakdown(breakdownName) {
				@Override
				public TaskBreakdown getTaskBreakdown(
						TaskFactory.Task parentTask) {
					ExtensionHandler.ExtensionFilter[] filters = xHandler
							.getExtensionFilter(breakdownName);
					List<TaskFactory.Task> tasks = new ArrayList<TaskFactory.Task>();
					for (ExtensionHandler.ExtensionFilter filter : filters) {
						DeleteGroupFromRepository.Args args = new DeleteGroupFromRepository.Args(
								((Task) parentTask).publicationSource, filter);
						DeleteGroupFromRepository subFactory = model
								.getTaskFactory(
										DeleteGroupFromRepository.class, args);
						tasks.add(subFactory.createRelativeTask(filter
								.getExtension()));
					}
