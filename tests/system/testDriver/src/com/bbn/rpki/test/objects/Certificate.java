...
public class Certificate extends CA_Obj {
	...
	Certificate(CA_Object parent, final long validityStartTime,
			final long validityEndTime, final String dirPath,
			final String nickname, final String siaPath, IPRangeList asList,
			IPRangeList ipv4, IPRangeList ipv6, String subjKeyFile,
			String... xargs) {
		super(xargs);
		...
		if (subjKeyFile != null) {
			if (subjKeyFile.equals(this.subjkeyfile)) {
				...
				if (DEBUG_ON) {
					System.out.println("Copying pre-specified key file: "
							+ subjKeyFile + " to " + this.subjkeyfile);
				}
			}
		} else {
			String pregeneratedKeyFileName = PregeneratedKeys
					.getPregeneratedKey();
			if (pregeneratedKeyFileName != null) {
				Util.copyfile(pregeneratedKeyFileName, this.subjkeyfile);
				if (DEBUG_ON) {
					System.out.println("Using pre-generated key for "
							+ this.subjkeyfile);
				}
			} else {
				Util.exec("gen_key", false, false, null, null, null,
						"../../cg/tools/gen_key", this.subjkeyfile, "2048");
				if (DEBUG_ON) {
					System.out.println("Creating new key for "
							+ this.subjkeyfile);
				}
			}
		}
		...
		this.outputfilename = dirPath + Util.b64encode_wrapper(this.ski)
				+ ".cer";
		if (DEBUG_ON) {
