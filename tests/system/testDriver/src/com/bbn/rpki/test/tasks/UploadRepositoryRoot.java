...
public class UploadRepositoryRoot extends TaskFactory {
	...
	protected class Task extends TaskFactory.Task {
		...
		public void run() {
			...
			for (com.bbn.rpki.test.tasks.TaskFactory.Task task : breakdown
					.getTaskBreakdown(this).getTasks()) {
				task.run();
			}
		}
	...
	protected void appendBreakdowns(List<Breakdown> list) {
		list.add(new Breakdown("byNode") {
			...
			public TaskBreakdown getTaskBreakdown(TaskFactory.Task task) {
				...
				UploadTrustAnchors uploadTrustAnchorsFactory = model
						.getTaskFactory(UploadTrustAnchors.class);
				String rootName = model.getRepositoryRootName(parentTask
