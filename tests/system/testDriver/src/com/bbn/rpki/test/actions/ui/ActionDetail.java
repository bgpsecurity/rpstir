 ...
 * 
 * At the top is a title stating the kind of action and its principal
 * attributes. Below are all the attributes of the action. An editor is provided
 * for each attribute depending on what kind of value the attribute has. Below
 * that is a tabbed pane with a tab for each epochEvent in which the action
 * plays a part. Constraints on the timing of the part of the action can be
 * selected.
 * 
 * @author tomlinso
 */
public class ActionDetail {
	...
	public void setAction(final AbstractAction action, EpochEvent epochEvent) {
		...
		savers.clear();
		this.action = action;
		String titleText = action == null ? "No Action Selected" : action
				.toString();
		...
		if (action != null) {
			...
			for (Map.Entry<String, Object> entry : attrs.entrySet()) {
				...
				if (value instanceof EpochEvent) {
					...
					if (epochEventsPane == null) {
						...
						gbc.gridwidth = 2;
						gbc.weightx = 1f;
						gbc.fill = component instanceof JScrollPane ? GridBagConstraints.BOTH
			...
			gbc.weighty = 1f;
			if (epochEventsPane != null) {
				epochEventsPane.getModel().addChangeListener(
						new ChangeListener() {
							...
							public void stateChanged(ChangeEvent e) {
								...
								if (index < 0) {
									...
									ActionDetail.this.epochEvent = epochEvents
											.get(index);
								}
							}
						});
	...
	private Component getTaskPathComponent(final TaskPath taskPath,
			final AbstractSetter setter) {
	...
	private Component getAllocationIdComponent(final AllocationId allocationId,
			final AbstractSetter setter) {
	 ...
	 * @return
	 */
	private Component getPairComponent(final Pair pair,
			final AbstractSetter setter) {
		AbstractFormatter formatter = new AbstractFormatter() {
	...
	private Component getIPRangeTypeComponent(IPRangeType value,
			final AbstractSetter setter) {
		final JComboBox<IPRangeType> box = new JComboBox<IPRangeType>(
				IPRangeType.values());
	...
	private Component getEpochEventComponent(EpochEvent value,
			final AbstractSetter setter) {
		...
		final JComboBox<EpochEvent> combo = new JComboBox<EpochEvent>(
				epochArray);
	 ...
	 * @return
	 */
	private Component getEpochEventsCoincidentComponent(
			final EpochEvent epochEvent) {
		Collection<EpochEvent> coincident = epochEvent.getCoincidentEpochs();
		Collection<EpochEvent> candidates = model
				.getPossibleCoincidentEpochs(epochEvent);
		return getEpochsComponent(epochEvent, "Coincident Epoch Events",
				coincident, candidates, new EpochCallback() {
	...
	private Component getEpochEventsBeforeComponent(final EpochEvent epochEvent) {
		Collection<EpochEvent> predecessors = epochEvent
				.getPredecessorEpochEvents();
		Collection<EpochEvent> candidates = model
				.getPossiblePredecessors(epochEvent);
		return getEpochsComponent(epochEvent, "Predecessor Epoch Events",
				predecessors, candidates, new EpochCallback() {
	...
	private Component getEpochEventsAfterComponent(final EpochEvent epochEvent) {
		Collection<EpochEvent> successors = epochEvent
				.getSuccessorEpochEvents();
		Collection<EpochEvent> candidates = model
				.getPossibleSuccessors(epochEvent);
		return getEpochsComponent(epochEvent, "Successor Epoch Events",
				successors, candidates, new EpochCallback() {
	...
	private Component getEpochsComponent(final EpochEvent epochEvent,
			String title, final Collection<EpochEvent> currentSelection,
			Collection<EpochEvent> availableEpochs, final EpochCallback cb) {
		return new EpochsComponent(model, epochEvent, title, currentSelection,
				availableEpochs, cb);
	}

	private Component getTypedPairComponent(final TypedPair typedPair,
			final AbstractSetter setter) {
	...
	private Component getPairListComponent(final TypedPairList list,
			final AbstractSetter setter) {
		...
		newButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				TypedPair newPair = new TypedPair(IPRangeType.ipv4, "p",
						BigInteger.ZERO);
				list.add(newPair);
	 ...
	 * @return
	 */
	private Component getStringComponent(Object value,
			final AbstractSetter setter) {
	 ...
	 * @return
	 */
	private Component getNumberComponent(Object value,
			final AbstractSetter setter) {
	...
	private void addTypedPair(final AbstractSetter setter,
			final TypedPairList list, final JPanel listData,
			GridBagConstraints gbc, final TypedPair element) {
	 ...
	 * @param component
	 */
	private void addTypedPairComponent(final JPanel listData,
			GridBagConstraints gbc, final Component component) {
		gbc.gridx = 0;
		gbc.fill = GridBagConstraints.BOTH;
		gbc.weightx = 1f;
