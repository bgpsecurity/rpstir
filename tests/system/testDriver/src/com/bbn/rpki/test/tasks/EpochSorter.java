...
/**
 * Starts with a collection of EpochEvents and groups coincident EpochEvents
 * into Epochs. Then imposes the constraints on the individual EpochEvents onto
 * the Epochs. Finally, the Epochs are sorted. Such that all predecessor Epochs
...
public class EpochSorter {
	 ...
	 * within the epoch.
	 * 
	 * @return the Epoch collection sorted into a proper succession of epoch
	 *         events.
	 */
	...
	/**
	 * The epochIndex of any Epoch is one greater than the maximum epochIndex of
	...
	/**
	 * Find EpochGroups coincident with the given epochEvent. Combine
	 * EpochGroups that are mutually coincident with the given epochEvent.
	...
	private Epoch findEpoch(EpochEvent epochEvent) {
		Epoch epoch = null;
		List<EpochEvent> pendingEpochEvents = new ArrayList<EpochEvent>(
				1 + epochEvent.getCoincidentEpochs().size());
		Collection<EpochEvent> coincidentEpochs = epochEvent
				.getCoincidentEpochs();
