...
public abstract class AllocateActionBase extends AbstractAction {
	...
	enum AttributeType {
		ISSUER("Issuer"), SUBJECT("Subject"), ALLOCATION_PUBLICATION_TIME(
				"Allocation Publication Time"), DEALLOCATION_PUBLICATION_TIME(
				"Deallocation Publication Time"), VALIDITY_START_TIME(
				"Validity Start Time"), VALIDITY_END_TIME("Validity End Time"), ALLOCATION_ID(
				"Allocation Id"), ALLOCATIONS("Allocations");
	 ...
	 * @param model
	 * @param pairs
	 *            the ranges or prefixes to be allocated
	 */
	public AllocateActionBase(CA_Object parent, AllocationId allocationId,
			Model model, TypedPair... pairs) {
		super(model);
		this.parent = parent;
		this.allocationId = allocationId;
		this.allocationPairs.addAll(Arrays.asList(pairs));
		allocationPublicationTime = new EpochEvent(this,
				PUBLICATION_TIME_OF_ALLOCATION);
		deallocationPublicationTime = new EpochEvent(this,
				PUBLICATION_TIME_OF_DEALLOCATION);
		validityStartTime = new EpochEvent(this,
				VALIDITY_START_TIME_OF_ALLOCATION);
	 ...
	 * @param actionContext
	 */
	public AllocateActionBase(Element element, Model model,
			ActionContext actionContext) {
		super(model);
		String parentCommonName = element.getAttributeValue(ATTR_PARENT_NAME);
		allocationId = AllocationId.get(element
				.getAttributeValue(ATTR_ALLOCATION_ID));
		...
		Element allocationPublicationTimeElement = element
				.getChild(AttributeType.ALLOCATION_PUBLICATION_TIME.name());
		Element deallocationPublicationTimeElement = element
				.getChild(AttributeType.DEALLOCATION_PUBLICATION_TIME.name());
		Element validityStartTimeElement = element
				.getChild(AttributeType.VALIDITY_START_TIME.name());
		Element validityEndTimeElement = element
				.getChild(AttributeType.VALIDITY_END_TIME.name());

		allocationPublicationTime = new EpochEvent(this,
				PUBLICATION_TIME_OF_ALLOCATION,
				allocationPublicationTimeElement, actionContext);
		deallocationPublicationTime = new EpochEvent(this,
				PUBLICATION_TIME_OF_DEALLOCATION,
				deallocationPublicationTimeElement, actionContext);
		validityStartTime = new EpochEvent(this,
				VALIDITY_START_TIME_OF_ALLOCATION, validityStartTimeElement,
				actionContext);
		validityEndTime = new EpochEvent(this, VALIDITY_END_TIME_OF_ALLOCATION,
				validityEndTimeElement, actionContext);
	...
	protected Element appendXML(Element element, ActionContext actionContext) {
		...
		element.addContent(allocationPublicationTime.toXML(
				AttributeType.ALLOCATION_PUBLICATION_TIME.name(), actionContext));
		element.addContent(deallocationPublicationTime.toXML(
				AttributeType.DEALLOCATION_PUBLICATION_TIME.name(),
				actionContext));
		element.addContent(validityStartTime.toXML(
				AttributeType.VALIDITY_START_TIME.name(), actionContext));
		element.addContent(validityEndTime.toXML(
				AttributeType.VALIDITY_END_TIME.name(), actionContext));
	...
	public Collection<EpochEvent> getAllEpochEvents() {
		return Arrays.asList(allocationPublicationTime, validityStartTime,
				deallocationPublicationTime, validityEndTime);
	}
	...
	public Map<String, Object> getAttributes() {
		...
		maybePutSubject(ret);
		ret.put(AttributeType.ALLOCATION_PUBLICATION_TIME.getDisplayName(),
				allocationPublicationTime);
		ret.put(AttributeType.VALIDITY_START_TIME.getDisplayName(),
				validityStartTime);
		ret.put(AttributeType.DEALLOCATION_PUBLICATION_TIME.getDisplayName(),
				deallocationPublicationTime);
		ret.put(AttributeType.VALIDITY_END_TIME.getDisplayName(),
				validityEndTime);
	...
	/**
	 * @see com.bbn.rpki.test.actions.AbstractAction#updateAttribute(java.lang.String,
	 *      java.lang.Object)
	 */
	@Override
	...
	public Collection<EpochEvent> getExecutionEpochs() {
		return Arrays.asList(allocationPublicationTime,
				deallocationPublicationTime);
	}
