 ...
 * 
 * There are two breakdowns: deleteFirst has two subtasks to delete and upload
 * in that order uploadFirst has two subtasks to upload and delete in that order
 * 
 * @author tomlinso
 */
public class UploadNode extends TaskFactory {
	protected class Task extends TaskFactory.Task {
		...
		private TaskFactory.Task getUploadTask() {
			if (uploadTask == null) {
				UploadNodeFiles factory = model
						.getTaskFactory(UploadNodeFiles.class);
				uploadTask = factory.createRelativeTask(model
						.getNodeName(nodeDir));
			}
			return uploadTask;
		}
		...
		private TaskFactory.Task getDeleteTask() {
			if (deleteTask == null) {
				...
				DeleteFromRepositoryNode factory = model
						.getTaskFactory(DeleteFromRepositoryNode.class);
		...
		protected String getLogDetail() {
			if (getSelectedTaskBreakdown() == null) {
				return String.format("%s upload %s and delete %s",
						nodeDir.getName(), getUploadTask().getLogDetail(),
						getDeleteTask().getLogDetail());
			}
	...
	protected void appendBreakdowns(List<Breakdown> list) {
		list.add(new Breakdown("deleteFirst") {
			...
			public TaskBreakdown getTaskBreakdown(TaskFactory.Task task) {
				...
				return new TaskBreakdown(getBreakdownName(), parentTask,
						parentTask.getDeleteTask(), parentTask.getUploadTask());
			}
		});
		list.add(new Breakdown("updateFirst") {
			...
			public TaskBreakdown getTaskBreakdown(TaskFactory.Task task) {
				Task parentTask = (Task) task;
				return new TaskBreakdown(getBreakdownName(), parentTask,
						parentTask.getUploadTask(), parentTask.getDeleteTask());
			}
		});
	}
