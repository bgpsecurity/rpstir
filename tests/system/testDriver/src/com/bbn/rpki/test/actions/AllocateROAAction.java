...
public class AllocateROAAction extends AllocateActionBase {
	 ...
	 */
	public AllocateROAAction(CA_Object parent, AllocationId allocationId,
	 ...
	 * @param actionContext
	 */
	public AllocateROAAction(Element element, Model model,
	...
	public void execute(EpochEvent epochEvent, TypescriptLogger logger) {
		...
		case PUBLICATION_TIME_OF_ALLOCATION:
			...
			eeObject = new EE_Object(validityStartTime.getEpoch()
					.getEpochTime(), validityEndTime.getEpoch().getEpochTime(),
					asid, allocationPairs.extract(IPRangeType.ipv4),
					allocationPairs.extract(IPRangeType.ipv6), "ROA-"
							+ allocationId, parent);
			Roa roa = new Roa(eeObject);
			if (logger != null) {
				logger.format(
						"Allocate ROA %s from %s to %s identified as %s%n",
						allocationPairs, parent, eeObject, allocationId);
			}
			break;
		case VALIDITY_END_TIME_OF_ALLOCATION:
			...
			if (logger != null) {
				logger.format(
						"Deallocate ROA %s from %s to %s identified as %s%n",
						allocationPairs, parent, eeObject, allocationId);
			}
			break;
		}
	...
	public String toString() {
		return String.format("Allocate ROA %s: %s from %s", allocationId,
				allocationPairs, parent.getNickname());
	}
